...
enum PREdits {
    ...
    static func checked(_ entities: [PRNoteEntity], map: PRTimeMap) throws -> [PRNoteEntity] {
        try validate(entities.map(\.note), clipLength: map.clipLength)
        let ids = Set(entities.map(\.id))
        guard ids.count == entities.count, !ids.contains(0) else { throw PREditError.invalidNote }
        return entities
    }
    static func validateFreshIDs(_ nextID: UInt64, count: Int, in entities: [PRNoteEntity]) throws {
        guard count >= 0, nextID > 0, UInt64(count) <= UInt64.max - nextID else { throw PREditError.invalidNote }
        let end = nextID + UInt64(count)
        guard !entities.contains(where: { $0.id >= nextID && $0.id < end }) else { throw PREditError.invalidNote }
    }
    static func move(_ entities: [PRNoteEntity], selected: Set<UInt64>, beats: Double, semitones: Int, map: PRTimeMap) throws -> [PRNoteEntity] {
        _ = try checked(entities, map: map)
    ...
    static func split(_ entities: [PRNoteEntity], selected: Set<UInt64>, at frame: UInt64,
                      nextID: UInt64, map: PRTimeMap) throws -> [PRNoteEntity] {
        _ = try checked(entities, map: map)
        let count = entities.filter { selected.contains($0.id) && frame > $0.note.startFrames &&
            frame < $0.note.startFrames + $0.note.lengthFrames }.count
        guard count <= PRLimits.noteCount - entities.count else { throw PREditError.tooManyNotes }
        try validateFreshIDs(nextID, count: count, in: entities)
        var output: [PRNoteEntity] = [], id = nextID
        for entity in entities {
            let note = entity.note, end = note.startFrames + note.lengthFrames
            guard selected.contains(entity.id), frame > note.startFrames, frame < end else {
                output.append(entity); continue
            }
            var left = entity; left.note.lengthFrames = frame - note.startFrames; output.append(left)
            var right = note; right.startFrames = frame; right.lengthFrames = end - frame
            output.append(PRNoteEntity(id: id, note: right)); id += 1
        }
        return try checked(output, map: map)
    }
