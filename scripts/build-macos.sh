...
if [ -f "$VST3_SDK_DIR/CMakeLists.txt" ] && [ -f "$DAW_ROOT/tools/vst3_scan_helper.cpp" ]; then VST3_HELPER_OPTION=ON; fi
if [ -f "$VST3_SDK_DIR/CMakeLists.txt" ] && [ -f "$DAW_ROOT/tools/vst3_runtime_helper.cpp" ]; then VST3_RUNTIME_HELPER_OPTION=ON; fi
cmake --preset debug -DCMAKE_OSX_DEPLOYMENT_TARGET=14.0 -DCMAKE_OSX_ARCHITECTURES=arm64 -DDAW_BUILD_VST3_SCAN_HELPER="$VST3_HELPER_OPTION" -DDAW_BUILD_VST3_RUNTIME_HELPER="$VST3_RUNTIME_HELPER_OPTION"
BUILD_TARGETS=(daw_core daw_au_scan_helper)
if [ "$VST3_HELPER_OPTION" = ON ]; then BUILD_TARGETS+=(daw_vst3_scan_helper); fi
if [ "$VST3_RUNTIME_HELPER_OPTION" = ON ]; then BUILD_TARGETS+=(daw_vst3_runtime_helper); fi
...
if pgrep -f "^$DAW_APP/Contents/MacOS/My DAW$" >/dev/null; then
  ...
  if pgrep -f "^$DAW_APP/Contents/MacOS/My DAW$" >/dev/null; then
    ...
    if pgrep -f "^$DAW_APP/Contents/MacOS/My DAW$" >/dev/null; then printf 'All three app build slots are running. Save and close one before rebuilding.\n' >&2; exit 1; fi
  fi
fi
mkdir -p "$DAW_APP/Contents/MacOS" "$DAW_APP/Contents/Resources/Workflows/vocal-preparation" "$DAW_APP/Contents/Resources/ThirdPartyNotices" "$DAW_APP/Contents/Resources/DesignSystem"
...
with open(path, "rb") as handle: data = plistlib.load(handle)
...
if [ "$VST3_HELPER_OPTION" = ON ]; then cp build/debug/daw_vst3_scan_helper "$DAW_APP/Contents/MacOS/daw_vst3_scan_helper"; cp third_party/notices/VST3-SDK.txt "$DAW_APP/Contents/Resources/ThirdPartyNotices/VST3-SDK.txt"; fi
if [ "$VST3_RUNTIME_HELPER_OPTION" = ON ]; then cp build/debug/daw_vst3_runtime_helper "$DAW_APP/Contents/MacOS/daw_vst3_runtime_helper"; fi
...
PIANO_ROLL_SOURCES=(apps/macos/PianoRoll/PianoRollModel.swift apps/macos/PianoRoll/PianoRollTiming.swift apps/macos/PianoRoll/PianoRollEdits.swift)
...
if [ "$VST3_HELPER_OPTION" = ON ]; then codesign --force --sign "$DAW_SIGNING_IDENTITY" "$DAW_APP/Contents/MacOS/daw_vst3_scan_helper"; fi
if [ "$VST3_RUNTIME_HELPER_OPTION" = ON ]; then codesign --force --sign "$DAW_SIGNING_IDENTITY" "$DAW_APP/Contents/MacOS/daw_vst3_runtime_helper"; fi
codesign --force --sign "$DAW_SIGNING_IDENTITY" "$DAW_APP"
codesign --verify --strict "$DAW_APP"
if [ "$DAW_SIGNING_IDENTITY" = "-" ]; then printf 'Local ad-hoc build: macOS may request file permissions again after an update.\n' >&2; fi
