...
template<class Fn> int guard(daw_session* s, Fn fn) noexcept {
    if (!s) return 1;
    try { fn(); s->error[0] = 0; return 0; }
    catch (const std::exception& e) { std::snprintf(s->error, sizeof(s->error), "%s", e.what()); }
    catch (...) { std::snprintf(s->error, sizeof(s->error), "Unknown core error"); }
    return 1;
}
uint64_t duration(daw_session* s) {
    uint64_t frames=0; for(const auto& t:s->model.state().tracks) for(const auto& region:t.regions) frames=std::max(frames,region.start+region.length); return frames;
}
void cancelPlaybackPreparation(daw_session* s) noexcept {
    ...
    std::unique_ptr<daw::Output> readyOutput;
    {
        ...
        if(status==PlaybackPreparationStatus::preparing||status==PlaybackPreparationStatus::ready)
            preparation->status.store(PlaybackPreparationStatus::canceled,std::memory_order_release);
    }
}
...
void beginPlaybackPreparation(daw_session* s) {
    ...
    if(s->output){
        if(s->output->renderer.playing.load(std::memory_order_acquire))return;
        s->output.reset();
    }
    auto permit=daw::tryAcquireBackgroundJob();
    if(!permit)throw daw::Error("Too many background jobs are active");
    ...
    std::thread worker([preparation,snapshot=std::move(snapshot)]() mutable {
        try {
            if(preparation->cancel.load(std::memory_order_acquire))return;
            auto output=daw::makeOutput();
            output->prepare(snapshot,preparation->startFrame,preparation->loopStart,preparation->loopEnd);
            std::lock_guard lock(preparation->publication);
            if(preparation->cancel.load(std::memory_order_acquire)||
               preparation->status.load(std::memory_order_acquire)!=PlaybackPreparationStatus::preparing)return;
    ...
    try { worker.detach(); }
    catch(...) { if(worker.joinable())worker.join();throw; }
...
void pollPlaybackPreparation(daw_session* s) {
    ...
    if(status==PlaybackPreparationStatus::preparing)return;
    ...
    if(status==PlaybackPreparationStatus::failed){
        std::string error;
        {std::lock_guard lock(preparation->publication);error=preparation->error;}
        if(s->playbackPreparation==preparation)s->playbackPreparation.reset();
        s->transientOutputState=7;
        throw daw::Error(error.empty()?"Playback preparation failed":error);
    }
    if(status!=PlaybackPreparationStatus::ready)return;
    if(preparation->generation!=s->playbackGeneration||preparation->revision!=s->model.state().revision){
        cancelPlaybackPreparation(s);s->transientOutputState=0;return;
    }
    std::unique_ptr<daw::Output> output;
    {
        std::lock_guard lock(preparation->publication);
...
void rebuildOutputAfterAutomationCommit(daw_session* s){
    if(!s->output)return;const bool resume=s->output->renderer.playing.load();const auto audible=s->output->renderer.audiblePositionFrames();s->output->stop();s->output.reset();s->selectedFrame=std::min(audible,duration(s));
...
constexpr daw::AutomationWriteMode automationMode(int32_t mode){
    switch(mode){case DAW_AUTOMATION_TOUCH:return daw::AutomationWriteMode::Touch;case DAW_AUTOMATION_LATCH:return daw::AutomationWriteMode::Latch;default:throw daw::Error("Unsupported automation write mode");}
}
template<size_t N> void copyText(char (&destination)[N],const std::string& source){std::memset(destination,0,N);std::memcpy(destination,source.data(),std::min(source.size(),N-1));}
std::vector<daw::WorkflowOperation> workflowOperations(const daw_workflow_operation* operations,uint32_t count){if(!operations||count==0||count>100)throw daw::Error("Workflow requires 1–100 operations");std::vector<daw::WorkflowOperation> result;result.reserve(count);for(uint32_t index=0;index<count;++index){const auto& item=operations[index];if(item.struct_size!=sizeof(daw_workflow_operation))throw daw::Error("Workflow operation ABI mismatch");daw::WorkflowOperation operation;operation.trackID=item.track_id;operation.gainDb=item.gain_db;if(item.kind==1){operation.kind=daw::WorkflowOperationKind::RenameTrack;const auto length=strnlen(item.name,sizeof(item.name));if(length==sizeof(item.name))throw daw::Error("Workflow name is not terminated");operation.name.assign(item.name,length);}else if(item.kind==2)operation.kind=daw::WorkflowOperationKind::SetTrackGain;else throw daw::Error("Unsupported workflow operation");result.push_back(std::move(operation));}return result;}
struct VocalPlan { std::vector<uint64_t> ids;std::vector<daw::TrackAnalysis> analyses;std::vector<daw::WorkflowOperation> operations;daw::WorkflowPreview preview; };
...
void validateInsertOwner(int32_t owner,uint64_t ownerID){
    switch(owner){
        case DAW_INSERT_OWNER_MASTER:if(ownerID)throw daw::Error("Master insert owner ID must be zero");return;
        case DAW_INSERT_OWNER_TRACK:case DAW_INSERT_OWNER_BUS:if(!ownerID)throw daw::Error("Track and bus insert owners require an ID");return;
        default:throw daw::Error("Unsupported insert owner");
    }
}
daw::PluginOwner pluginOwner(int32_t owner){
    switch(owner){case DAW_INSERT_OWNER_MASTER:return daw::PluginOwner::Master;case DAW_INSERT_OWNER_TRACK:return daw::PluginOwner::Track;case DAW_INSERT_OWNER_BUS:return daw::PluginOwner::Bus;default:throw daw::Error("Unsupported insert owner");}
}
const std::vector<daw::PluginInsert>& insertsFor(const daw::State& state,int32_t owner,uint64_t ownerID){
    validateInsertOwner(owner,ownerID);
    switch(owner){
        case DAW_INSERT_OWNER_MASTER:return state.masterInserts;
        case DAW_INSERT_OWNER_TRACK:{const auto track=std::find_if(state.tracks.begin(),state.tracks.end(),[&](const auto& item){return item.id==ownerID;});if(track==state.tracks.end())throw daw::Error("Track not found");return track->inserts;}
        case DAW_INSERT_OWNER_BUS:{const auto bus=std::find_if(state.buses.begin(),state.buses.end(),[&](const auto& item){return item.id==ownerID;});if(bus==state.buses.end())throw daw::Error("Bus not found");return bus->inserts;}
        default:throw daw::Error("Unsupported insert owner");
    }
}
const daw::PluginInsert& insertFor(const daw::State& state,int32_t owner,uint64_t ownerID,uint64_t insertID){
    const auto& inserts=insertsFor(state,owner,ownerID);const auto insert=std::find_if(inserts.begin(),inserts.end(),[&](const auto& item){return item.id==insertID;});if(insert==inserts.end())throw daw::Error("Insert not found");return *insert;
}
uint32_t insertHostingFormat(const daw::PluginInsert& plugin) noexcept {
    if(daw::isVst3PluginInsert(plugin)) return DAW_INSERT_HOSTING_FORMAT_VST3;
#ifdef __APPLE__
    ...
    if(const auto component=AudioComponentFindNext(nullptr,&description)) {
        ...
        if(AudioComponentGetDescription(component,&actual)==noErr &&
           (actual.componentFlags & kAudioComponentFlag_IsV3AudioUnit)) return DAW_INSERT_HOSTING_FORMAT_AUV3;
    }
#endif
...
daw::PluginHostingMode hostingMode(uint32_t mode) {
    switch(mode) {
        case DAW_INSERT_HOSTING_MODE_IN_PROCESS:return daw::PluginHostingMode::InProcess;
        case DAW_INSERT_HOSTING_MODE_OUT_OF_PROCESS:return daw::PluginHostingMode::OutOfProcess;
        default:throw daw::Error("Unsupported plug-in hosting mode");
    }
}
uint32_t bridgeHostingMode(daw::PluginHostingMode mode) {
    switch(mode) {
        case daw::PluginHostingMode::InProcess:return DAW_INSERT_HOSTING_MODE_IN_PROCESS;
        case daw::PluginHostingMode::OutOfProcess:return DAW_INSERT_HOSTING_MODE_OUT_OF_PROCESS;
    }
...
void writeInsertHostingStatus(const daw::PluginInsert& plugin,daw_insert_hosting_status* out) {
    if(!out || out->struct_size!=sizeof(daw_insert_hosting_status)) throw daw::Error("Insert hosting status ABI mismatch");
    *out={};out->struct_size=sizeof(daw_insert_hosting_status);out->version=DAW_INSERT_HOSTING_STATUS_VERSION;
...
void readChannelMeter(const daw::State& state,const daw::Renderer& renderer,int32_t owner,uint64_t ownerID,float& left,float& right){
    validateInsertOwner(owner,ownerID);
    if(!renderer.playing.load(std::memory_order_acquire))return;
    switch(owner){
        case DAW_INSERT_OWNER_MASTER:renderer.masterMeter(left,right);return;
        case DAW_INSERT_OWNER_TRACK:{
            const auto track=std::find_if(state.tracks.begin(),state.tracks.end(),[&](const auto& item){return item.id==ownerID;});
            if(track==state.tracks.end())throw daw::Error("Track not found");
            const auto index=static_cast<size_t>(std::distance(state.tracks.begin(),track));
            if(!renderer.trackMeter(index,left,right)){left=0;right=0;}
            return;
        }
        case DAW_INSERT_OWNER_BUS:{
            const auto bus=std::find_if(state.buses.begin(),state.buses.end(),[&](const auto& item){return item.id==ownerID;});
            if(bus==state.buses.end())throw daw::Error("Bus not found");
            const auto index=static_cast<size_t>(std::distance(state.buses.begin(),bus));
            if(!renderer.busMeter(index,left,right)){left=0;right=0;}
            return;
        }
        default:throw daw::Error("Unsupported insert owner");
    }
...
void writePlugin(daw_session* s,const daw::PluginInsert& plugin,daw_plugin* out){
    if(!out||out->struct_size!=sizeof(daw_plugin))throw daw::Error("Plugin ABI mismatch");*out={};out->struct_size=sizeof(daw_plugin);out->id=plugin.id;out->type=plugin.type;out->subtype=plugin.subtype;out->manufacturer=plugin.manufacturer;out->bypassed=plugin.bypassed;out->latency_frames=plugin.latencyFrames;out->available=insertAvailable(s,plugin)?1:0;copyText(out->name,plugin.name);
}
}
extern "C" {
...
int daw_get_track_volume_automation_count(daw_session* s,uint64_t trackID,uint32_t* count){return guard(s,[&]{
    ...
    if(track==s->model.state().tracks.end())throw daw::Error("Track not found");*count=static_cast<uint32_t>(track->volumeAutomation.size());
});}
int daw_get_track_volume_automation_point(daw_session* s,uint64_t trackID,uint32_t index,daw_automation_point* out){return guard(s,[&]{
    if(!out||out->struct_size!=sizeof(daw_automation_point))throw daw::Error("Automation point ABI mismatch");const auto track=std::find_if(s->model.state().tracks.begin(),s->model.state().tracks.end(),[&](const auto& item){return item.id==trackID;});
    if(track==s->model.state().tracks.end()||index>=track->volumeAutomation.size())throw daw::Error("Automation point index out of range");const auto& point=track->volumeAutomation[index];*out={sizeof(daw_automation_point),point.frame,point.gainDb};
});}
int daw_upsert_track_volume_automation_point(daw_session* s,uint64_t trackID,uint64_t frame,double gain,uint64_t rev){return guard(s,[&]{s->model.upsertTrackVolumeAutomation(trackID,frame,gain,rev);cancelStalePlaybackPreparation(s);});}
int daw_remove_track_volume_automation_point(daw_session* s,uint64_t trackID,uint64_t frame,uint64_t rev){return guard(s,[&]{s->model.removeTrackVolumeAutomation(trackID,frame,rev);cancelStalePlaybackPreparation(s);});}
int daw_get_track_pan_automation_count(daw_session* s,uint64_t trackID,uint32_t* count){return guard(s,[&]{if(!count)throw daw::Error("Missing automation point count");const auto track=std::find_if(s->model.state().tracks.begin(),s->model.state().tracks.end(),[&](const auto& item){return item.id==trackID;});if(track==s->model.state().tracks.end())throw daw::Error("Track not found");*count=static_cast<uint32_t>(track->panAutomation.size());});}
int daw_get_track_pan_automation_point(daw_session* s,uint64_t trackID,uint32_t index,daw_automation_point* out){return guard(s,[&]{if(!out||out->struct_size!=sizeof(daw_automation_point))throw daw::Error("Automation point ABI mismatch");const auto track=std::find_if(s->model.state().tracks.begin(),s->model.state().tracks.end(),[&](const auto& item){return item.id==trackID;});if(track==s->model.state().tracks.end()||index>=track->panAutomation.size())throw daw::Error("Automation point index out of range");const auto& point=track->panAutomation[index];*out={sizeof(daw_automation_point),point.frame,point.gainDb};});}
int daw_upsert_track_pan_automation_point(daw_session* s,uint64_t trackID,uint64_t frame,double pan,uint64_t rev){return guard(s,[&]{s->model.upsertTrackPanAutomation(trackID,frame,pan,rev);cancelStalePlaybackPreparation(s);});}
int daw_remove_track_pan_automation_point(daw_session* s,uint64_t trackID,uint64_t frame,uint64_t rev){return guard(s,[&]{s->model.removeTrackPanAutomation(trackID,frame,rev);cancelStalePlaybackPreparation(s);});}
int daw_get_bus_gain_automation_count(daw_session* s,uint64_t busID,uint32_t* count){return guard(s,[&]{if(!count)throw daw::Error("Missing automation point count");const auto bus=std::find_if(s->model.state().buses.begin(),s->model.state().buses.end(),[&](const auto& item){return item.id==busID;});if(bus==s->model.state().buses.end())throw daw::Error("Bus not found");*count=static_cast<uint32_t>(bus->gainAutomation.size());});}
int daw_get_bus_gain_automation_point(daw_session* s,uint64_t busID,uint32_t index,daw_automation_point* out){return guard(s,[&]{if(!out||out->struct_size!=sizeof(daw_automation_point))throw daw::Error("Automation point ABI mismatch");const auto bus=std::find_if(s->model.state().buses.begin(),s->model.state().buses.end(),[&](const auto& item){return item.id==busID;});if(bus==s->model.state().buses.end()||index>=bus->gainAutomation.size())throw daw::Error("Automation point index out of range");const auto& point=bus->gainAutomation[index];*out={sizeof(daw_automation_point),point.frame,point.gainDb};});}
int daw_upsert_bus_gain_automation_point(daw_session* s,uint64_t busID,uint64_t frame,double gain,uint64_t rev){return guard(s,[&]{s->model.upsertBusGainAutomation(busID,frame,gain,rev);cancelStalePlaybackPreparation(s);});}
int daw_remove_bus_gain_automation_point(daw_session* s,uint64_t busID,uint64_t frame,uint64_t rev){return guard(s,[&]{s->model.removeBusGainAutomation(busID,frame,rev);cancelStalePlaybackPreparation(s);});}
int daw_get_master_gain_automation_count(daw_session* s,uint32_t* count){return guard(s,[&]{if(!count)throw daw::Error("Missing automation point count");*count=static_cast<uint32_t>(s->model.state().masterGainAutomation.size());});}
int daw_get_master_gain_automation_point(daw_session* s,uint32_t index,daw_automation_point* out){return guard(s,[&]{if(!out||out->struct_size!=sizeof(daw_automation_point))throw daw::Error("Automation point ABI mismatch");if(index>=s->model.state().masterGainAutomation.size())throw daw::Error("Automation point index out of range");const auto& point=s->model.state().masterGainAutomation[index];*out={sizeof(daw_automation_point),point.frame,point.gainDb};});}
int daw_upsert_master_gain_automation_point(daw_session* s,uint64_t frame,double gain,uint64_t rev){return guard(s,[&]{s->model.upsertMasterGainAutomation(frame,gain,rev);cancelStalePlaybackPreparation(s);});}
int daw_remove_master_gain_automation_point(daw_session* s,uint64_t frame,uint64_t rev){return guard(s,[&]{s->model.removeMasterGainAutomation(frame,rev);cancelStalePlaybackPreparation(s);});}
int daw_begin_automation_gesture(daw_session* s,int32_t target,uint64_t targetID,int32_t mode,uint64_t rev){return guard(s,[&]{s->model.beginAutomationGesture(automationTarget(target),targetID,automationMode(mode),rev);});}
int daw_write_automation_gesture(daw_session* s,uint64_t frame,double value){return guard(s,[&]{s->model.writeAutomationGesture(frame,value);});}
int daw_end_automation_gesture(daw_session* s,uint64_t endFrame,uint64_t rev){return guard(s,[&]{s->model.endAutomationGesture(endFrame,rev);rebuildOutputAfterAutomationCommit(s);});}
void daw_cancel_automation_gesture(daw_session* s){if(s)s->model.cancelAutomationGesture();}
int daw_begin_insert_parameter_automation_gesture(daw_session* s,int32_t owner,uint64_t ownerID,uint64_t insertID,uint32_t parameterID,const char* name,int32_t mode,uint64_t rev){return guard(s,[&]{
    validateInsertOwner(owner,ownerID);if(s->pluginParameterGesture)throw daw::Error("Plug-in parameter automation gesture is already active");(void)insertFor(s->model.state(),owner,ownerID,insertID);const auto parameterName=name?std::string(name):std::string{};s->model.beginPluginParameterAutomationGesture(pluginOwner(owner),ownerID,insertID,parameterID,parameterName,automationMode(mode),rev);s->pluginParameterGesture={insertID,parameterID};
});}
int daw_write_insert_parameter_automation_gesture(daw_session* s,uint64_t frame,double normalized){return guard(s,[&]{
    if(!s->pluginParameterGesture)throw daw::Error("No plug-in parameter automation gesture is active");if(!std::isfinite(normalized)||normalized<0||normalized>1)throw daw::Error("Plug-in automation value must be normalized 0…1");s->model.writePluginParameterAutomationGesture(frame,normalized);if(s->output){const auto live=*s->pluginParameterGesture;if(!s->output->renderer.updatePluginParameterTouch(live.pluginID,live.parameterID,static_cast<float>(normalized)))s->output->renderer.beginPluginParameterTouch(live.pluginID,live.parameterID,static_cast<float>(normalized));}
});}
int daw_end_insert_parameter_automation_gesture(daw_session* s,uint64_t endFrame,uint64_t rev){return guard(s,[&]{
    if(!s->pluginParameterGesture)throw daw::Error("No plug-in parameter automation gesture is active");const auto live=*s->pluginParameterGesture;s->model.endPluginParameterAutomationGesture(endFrame,rev);if(s->output)s->output->renderer.endPluginParameterTouch(live.pluginID,live.parameterID);s->pluginParameterGesture.reset();rebuildOutputAfterAutomationCommit(s);
});}
void daw_cancel_insert_parameter_automation_gesture(daw_session* s){if(!s)return;s->model.cancelPluginParameterAutomationGesture();if(s->output)s->output->renderer.cancelPluginParameterTouch();s->pluginParameterGesture.reset();}
int daw_scan_supported_au(daw_session* s,uint32_t* count){return guard(s,[&]{if(!count)throw daw::Error("Missing Audio Unit count");s->auCatalog=daw::supportedAudioUnits();*count=static_cast<uint32_t>(s->auCatalog.size());});}
daw_au_scan_job* daw_begin_installed_au_scan(const char* helperPath,uint32_t timeoutMs){
#ifdef __APPLE__
    try{if(!helperPath||!*helperPath||timeoutMs<100||timeoutMs>30000)return nullptr;auto job=std::make_unique<daw_au_scan_job>();const std::string path(helperPath);job->future=std::async(std::launch::async,[path,timeoutMs]{return daw::scanAudioUnitsIsolated(path,std::chrono::milliseconds(timeoutMs));});return job.release();}catch(...){return nullptr;}
#else
    (void)helperPath;(void)timeoutMs;return nullptr;
#endif
}
int daw_poll_installed_au_scan(daw_au_scan_job* job,daw_au_scan_status* out){
#ifdef __APPLE__
    if(!job||!out||out->struct_size!=sizeof(daw_au_scan_status))return 1;*out={};out->struct_size=sizeof(daw_au_scan_status);if(!job->result){if(job->future.wait_for(std::chrono::milliseconds(0))!=std::future_status::ready){out->status=0;return 0;}try{job->result=job->future.get();}catch(const std::exception& error){out->status=2;std::snprintf(out->error,sizeof(out->error),"%s",error.what());return 0;}catch(...){out->status=2;std::snprintf(out->error,sizeof(out->error),"Unknown scanner error");return 0;}}
    out->available_count=static_cast<uint32_t>(job->result->available.size());out->quarantined_count=static_cast<uint32_t>(job->result->quarantined.size());if(!job->result->helperError.empty()){out->status=2;std::snprintf(out->error,sizeof(out->error),"%s",job->result->helperError.c_str());}else out->status=1;return 0;
#else
    (void)job;(void)out;return 1;
#endif
}
int daw_apply_installed_au_scan(daw_session* s,daw_au_scan_job* job,uint32_t* available,uint32_t* quarantined){return guard(s,[&]{
#ifdef __APPLE__
    if(!job||!available||!quarantined||!job->result||!job->result->helperError.empty())throw daw::Error("Audio Unit scan is not ready");s->auCatalog=job->result->available;*available=static_cast<uint32_t>(job->result->available.size());*quarantined=static_cast<uint32_t>(job->result->quarantined.size());
#else
    (void)job;(void)available;(void)quarantined;throw daw::Error("Audio Unit scanning requires macOS");
#endif
});}
int daw_save_installed_au_scan_cache(daw_session* s,daw_au_scan_job* job,const char* path){return guard(s,[&]{
#ifdef __APPLE__
    if(!job||!job->result||!job->result->helperError.empty())throw daw::Error("Audio Unit scan is not ready");const auto now=std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count();const auto cache=daw::makeAudioUnitScanCache(*job->result,static_cast<uint64_t>(now));std::string error;if(!daw::writeAudioUnitScanCache(cache,required(path),error))throw daw::Error(error);
#else
    (void)job;(void)path;throw daw::Error("Audio Unit scanning requires macOS");
#endif
});}
int daw_load_installed_au_scan_cache(daw_session* s,const char* helperPath,const char* path,uint32_t* available,uint32_t* quarantined,uint32_t* invalidated){return guard(s,[&]{
#ifdef __APPLE__
    if(!available||!quarantined||!invalidated)throw daw::Error("Missing Audio Unit cache counts");std::string error;const auto cache=daw::readAudioUnitScanCache(required(path),error);if(!error.empty())throw daw::Error(error);s->auCatalog.clear();*available=0;*quarantined=0;*invalidated=0;if(!cache)return;const auto current=daw::enumerateAudioUnitsIsolated(required(helperPath),std::chrono::milliseconds(1500));const auto freshness=daw::freshAudioUnitScanCache(*cache,current);*invalidated=freshness.invalidatedEntries;for(const auto& entry:freshness.fresh.entries){if(entry.available){s->auCatalog.push_back(entry.descriptor);++*available;}else ++*quarantined;}if(freshness.invalidatedEntries){if(!daw::writeAudioUnitScanCache(freshness.fresh,path,error))throw daw::Error(error);}
#else
...
void daw_release_installed_au_scan(daw_au_scan_job* job){delete job;}
daw_vst3_scan_job* daw_begin_installed_vst3_scan(const char* helperPath,uint32_t timeoutMs){
#ifdef __APPLE__
    try{if(!helperPath||!*helperPath||timeoutMs<100||timeoutMs>30000)return nullptr;auto job=std::make_unique<daw_vst3_scan_job>();const std::string path(helperPath);job->future=std::async(std::launch::async,[path,timeoutMs]{return daw::scanVst3PluginsIsolated(path,std::chrono::milliseconds(timeoutMs));});return job.release();}catch(...){return nullptr;}
#else
    (void)helperPath;(void)timeoutMs;return nullptr;
#endif
}
...
#ifdef __APPLE__
    if(!job||!out||out->struct_size!=sizeof(daw_vst3_scan_status))return 1;*out={};out->struct_size=sizeof(daw_vst3_scan_status);if(!job->result){if(job->future.wait_for(std::chrono::milliseconds(0))!=std::future_status::ready){out->status=0;return 0;}try{job->result=job->future.get();}catch(const std::exception& error){out->status=2;copyText(out->error,std::string(error.what()));return 0;}catch(...){out->status=2;copyText(out->error,std::string("Unknown VST3 scanner error"));return 0;}}
    out->available_count=static_cast<uint32_t>(job->result->available.size());out->quarantined_count=static_cast<uint32_t>(job->result->quarantined.size());if(!job->result->helperError.empty()){out->status=2;copyText(out->error,job->result->helperError);}else out->status=1;return 0;
#else
    (void)job;(void)out;return 1;
#endif
}
int daw_apply_installed_vst3_scan(daw_session* s,daw_vst3_scan_job* job,uint32_t* available,uint32_t* quarantined){return guard(s,[&]{
#ifdef __APPLE__
    if(!job||!available||!quarantined||!job->result||!job->result->helperError.empty())throw daw::Error("VST3 scan is not ready");const auto now=std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count();const auto cache=daw::makeVst3ScanCache(*job->result,static_cast<uint64_t>(now));s->vst3Catalog=cache.entries;*available=static_cast<uint32_t>(job->result->available.size());*quarantined=static_cast<uint32_t>(job->result->quarantined.size());
#else
...
#ifdef __APPLE__
    try{if(!job||!job->result||!job->result->helperError.empty()||!path) return 1;const auto now=std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count();const auto cache=daw::makeVst3ScanCache(*job->result,static_cast<uint64_t>(now));std::string error;return daw::writeVst3ScanCache(cache,path,error)?0:1;}catch(...){return 1;}
#else
    (void)job;(void)path;return 1;
#endif
}
int daw_load_installed_vst3_scan_cache(daw_session* s,const char* helperPath,const char* path,uint32_t* available,uint32_t* quarantined,uint32_t* invalidated){return guard(s,[&]{
#ifdef __APPLE__
    if(!available||!quarantined||!invalidated)throw daw::Error("Missing VST3 cache counts");std::string error;const auto cache=daw::readVst3ScanCache(required(path),error);if(!error.empty())throw daw::Error(error);s->vst3Catalog.clear();*available=0;*quarantined=0;*invalidated=0;if(!cache)return;const auto current=daw::enumerateVst3PluginsIsolated(required(helperPath),std::chrono::milliseconds(1500));const auto freshness=daw::freshVst3ScanCache(*cache,current);if(!current.helperError.empty())throw daw::Error(current.helperError);*invalidated=freshness.invalidatedEntries;s->vst3Catalog=freshness.fresh.entries;for(const auto& entry:s->vst3Catalog){if(entry.available)++*available;else ++*quarantined;}if(freshness.invalidatedEntries&&!daw::writeVst3ScanCache(freshness.fresh,path,error))throw daw::Error(error);
#else
...
int daw_add_insert_vst3(daw_session* s,int32_t owner,uint64_t ownerID,uint32_t index,uint64_t rev){return guard(s,[&]{
    validateInsertOwner(owner,ownerID);
    if(index>=s->vst3Catalog.size())throw daw::Error("VST3 catalog index out of range");const auto& item=s->vst3Catalog[index];if(!item.available)throw daw::Error("Selected VST3 is quarantined or unavailable");const auto classID=daw::parseTextualVst3Fuid(item.plugin.classId);if(!classID)throw daw::Error("VST3 catalog FUID is invalid");
    daw::Vst3StateEnvelope envelope;envelope.descriptor.format=daw::PluginFormat::VST3;envelope.descriptor.vst3ClassFuid=*classID;envelope.descriptor.modulePath=item.plugin.modulePath;envelope.descriptor.vendor=item.plugin.vendor;envelope.descriptor.version=item.plugin.version;envelope.descriptor.fingerprint=item.plugin.moduleFingerprint;
    daw::PluginInsert insert{0,0,0,0,item.plugin.name,false,0,daw::encodeVst3StateEnvelope(envelope),{}};const auto snapshot=daw::snapshotVst3Effect(insert);insert.latencyFrames=snapshot.latencyFrames;insert.state=snapshot.state;
    switch(owner){case DAW_INSERT_OWNER_MASTER:if(ownerID)throw daw::Error("Master insert owner ID must be zero");s->model.addMasterInsert(std::move(insert),rev);break;case DAW_INSERT_OWNER_TRACK:s->model.addTrackInsert(ownerID,std::move(insert),rev);break;case DAW_INSERT_OWNER_BUS:s->model.addBusInsert(ownerID,std::move(insert),rev);break;default:throw daw::Error("Unsupported insert owner");}resetTransport(s);
});}
int daw_add_insert_au(daw_session* s,int32_t owner,uint64_t ownerID,uint32_t type,uint32_t subtype,uint32_t manufacturer,uint64_t rev){return guard(s,[&]{
    validateInsertOwner(owner,ownerID);
    const auto found=std::find_if(s->auCatalog.begin(),s->auCatalog.end(),[&](const auto& item){return item.type==type&&item.subtype==subtype&&item.manufacturer==manufacturer;});if(found==s->auCatalog.end())throw daw::Error("Scan and choose a supported Apple Audio Unit");auto snapshot=daw::snapshotAudioUnit(*found);daw::PluginInsert insert{0,type,subtype,manufacturer,std::move(snapshot.name),false,snapshot.latencyFrames,std::move(snapshot.state),{}};
    switch(owner){case DAW_INSERT_OWNER_MASTER:if(ownerID)throw daw::Error("Master insert owner ID must be zero");s->model.addMasterInsert(std::move(insert),rev);break;case DAW_INSERT_OWNER_TRACK:s->model.addTrackInsert(ownerID,std::move(insert),rev);break;case DAW_INSERT_OWNER_BUS:s->model.addBusInsert(ownerID,std::move(insert),rev);break;default:throw daw::Error("Unsupported insert owner");}resetTransport(s);
});}
int daw_get_insert_count(daw_session* s,int32_t owner,uint64_t ownerID,uint32_t* count){return guard(s,[&]{
    if(!count)throw daw::Error("Missing insert count");*count=static_cast<uint32_t>(insertsFor(s->model.state(),owner,ownerID).size());
});}
...
int daw_set_insert_hosting_mode(daw_session* s,int32_t owner,uint64_t ownerID,uint64_t insertID,uint32_t mode,uint64_t rev){return guard(s,[&]{
    ...
    if(!(supported&flag))throw daw::Error("Requested plug-in hosting mode is unavailable for this insert");
    switch(owner){
        case DAW_INSERT_OWNER_MASTER:s->model.setMasterInsertHostingMode(insertID,requested,rev);break;
        case DAW_INSERT_OWNER_TRACK:s->model.setTrackInsertHostingMode(ownerID,insertID,requested,rev);break;
        case DAW_INSERT_OWNER_BUS:s->model.setBusInsertHostingMode(ownerID,insertID,requested,rev);break;
        default:throw daw::Error("Unsupported insert owner");
    }
    resetTransport(s);
});}
...
int daw_move_insert(daw_session* s,int32_t owner,uint64_t ownerID,uint64_t insertID,uint32_t index,uint64_t rev){return guard(s,[&]{validateInsertOwner(owner,ownerID);switch(owner){case DAW_INSERT_OWNER_MASTER:s->model.moveMasterInsert(insertID,index,rev);break;case DAW_INSERT_OWNER_TRACK:s->model.moveTrackInsert(ownerID,insertID,index,rev);break;case DAW_INSERT_OWNER_BUS:s->model.moveBusInsert(ownerID,insertID,index,rev);break;default:throw daw::Error("Unsupported insert owner");}resetTransport(s);});}
int daw_remove_insert(daw_session* s,int32_t owner,uint64_t ownerID,uint64_t insertID,uint64_t rev){return guard(s,[&]{validateInsertOwner(owner,ownerID);switch(owner){case DAW_INSERT_OWNER_MASTER:s->model.removeMasterInsert(insertID,rev);break;case DAW_INSERT_OWNER_TRACK:s->model.removeTrackInsert(ownerID,insertID,rev);break;case DAW_INSERT_OWNER_BUS:s->model.removeBusInsert(ownerID,insertID,rev);break;default:throw daw::Error("Unsupported insert owner");}resetTransport(s);});}
int daw_get_insert_parameter_automation_count(daw_session* s,int32_t owner,uint64_t ownerID,uint64_t insertID,uint32_t parameterID,uint32_t* count){return guard(s,[&]{
    if(!count)throw daw::Error("Missing plug-in automation point count");const auto& insert=insertFor(s->model.state(),owner,ownerID,insertID);const auto lane=std::find_if(insert.parameterAutomation.begin(),insert.parameterAutomation.end(),[&](const auto& item){return item.parameterID==parameterID;});*count=lane==insert.parameterAutomation.end()?0:static_cast<uint32_t>(lane->points.size());
});}
int daw_get_insert_parameter_automation_point(daw_session* s,int32_t owner,uint64_t ownerID,uint64_t insertID,uint32_t parameterID,uint32_t index,daw_plugin_parameter_automation_point* out){return guard(s,[&]{
    if(!out||out->struct_size!=sizeof(daw_plugin_parameter_automation_point))throw daw::Error("Plug-in automation point ABI mismatch");const auto& insert=insertFor(s->model.state(),owner,ownerID,insertID);const auto lane=std::find_if(insert.parameterAutomation.begin(),insert.parameterAutomation.end(),[&](const auto& item){return item.parameterID==parameterID;});if(lane==insert.parameterAutomation.end()||index>=lane->points.size())throw daw::Error("Plug-in automation point index out of range");const auto& point=lane->points[index];*out={sizeof(daw_plugin_parameter_automation_point),point.frame,point.normalizedValue};
});}
int daw_upsert_insert_parameter_automation_point(daw_session* s,int32_t owner,uint64_t ownerID,uint64_t insertID,uint32_t parameterID,const char* name,uint64_t frame,double normalized,uint64_t rev){return guard(s,[&]{
    validateInsertOwner(owner,ownerID);if(!std::isfinite(normalized)||normalized<0||normalized>1)throw daw::Error("Plug-in automation value must be normalized 0…1");const auto parameterName=name?std::string(name):std::string{};s->model.upsertPluginParameterAutomation(pluginOwner(owner),ownerID,insertID,parameterID,parameterName,frame,normalized,rev);resetTransport(s);
});}
int daw_remove_insert_parameter_automation_point(daw_session* s,int32_t owner,uint64_t ownerID,uint64_t insertID,uint32_t parameterID,uint64_t frame,uint64_t rev){return guard(s,[&]{
    validateInsertOwner(owner,ownerID);s->model.removePluginParameterAutomation(pluginOwner(owner),ownerID,insertID,parameterID,frame,rev);resetTransport(s);
});}
int daw_preview_workflow(daw_session* s,const daw_workflow_operation* operations,uint32_t count,uint64_t rev,daw_workflow_change* changes,uint32_t capacity,uint32_t* changeCount,uint64_t* afterRevision){return guard(s,[&]{if(!changeCount||!afterRevision||(!changes&&capacity))throw daw::Error("Invalid workflow preview output");const auto preview=s->model.previewWorkflow(workflowOperations(operations,count),rev);std::vector<std::pair<daw::WorkflowTrackSummary,daw::WorkflowTrackSummary>> changed;for(size_t index=0;index<preview.before.size();++index)if(preview.before[index]!=preview.after[index])changed.push_back({preview.before[index],preview.after[index]});*changeCount=static_cast<uint32_t>(changed.size());*afterRevision=preview.afterRevision;if(!changes)return;if(capacity<changed.size())throw daw::Error("Workflow preview buffer is too small");for(size_t index=0;index<changed.size();++index){auto& out=changes[index];if(out.struct_size!=sizeof(daw_workflow_change))throw daw::Error("Workflow preview ABI mismatch");out={};out.struct_size=sizeof(daw_workflow_change);out.track_id=changed[index].first.id;out.before_gain_db=changed[index].first.gainDb;out.after_gain_db=changed[index].second.gainDb;std::memcpy(out.before_name,changed[index].first.name.data(),changed[index].first.name.size());std::memcpy(out.after_name,changed[index].second.name.data(),changed[index].second.name.size());}});}
...
int daw_set_record_preroll(daw_session* s,uint64_t frames){return guard(s,[&]{
    if(frames>48000*30)throw daw::Error("Pre-roll must be 0-30 seconds");
...
daw_save_job* daw_begin_save(daw_session* s,const char* path) {
    ...
    guard(s,[&]{auto handle=std::make_unique<daw_save_job>(); handle->result=daw::startSave(s->model.state(),required(path)); job=handle.release();});
    return job;
}
int daw_poll_save(daw_save_job* job,daw_save_status* out) {
    if(!job || !out || out->struct_size!=sizeof(daw_save_status)) return 1;
    auto& result=*job->result; out->status=result.status.load(std::memory_order_acquire); out->revision=result.revision;
    std::memset(out->error,0,sizeof(out->error));
    if(out->status==2) std::memcpy(out->error,result.error,sizeof(out->error));
    return 0;
}
...
int daw_poll_export(daw_export_job* job,daw_export_status* out) {
    ...
    std::memset(out->error,0,sizeof(out->error));if(out->status==2)std::memcpy(out->error,result.error,sizeof(out->error));return 0;
}
void daw_cancel_export(daw_export_job* job){if(job)job->result->cancel.store(true,std::memory_order_release);}
void daw_release_export(daw_export_job* job){delete job;}
daw_dawproject_job* daw_begin_dawproject_export(daw_session* s,const char* path,double tempo,uint32_t numerator,uint32_t denominator,const char* title) {
    daw_dawproject_job* job=nullptr;guard(s,[&]{
        auto handle=std::make_unique<daw_dawproject_job>();
        handle->result=daw::dawproject::startExport(s->model.state(),required(path),
            {tempo,numerator,denominator},required(title),"1.15.0");
        job=handle.release();
    });return job;
}
int daw_poll_dawproject_export(daw_dawproject_job* job,daw_dawproject_status* out) {
    if(!job||!out||out->struct_size!=sizeof(daw_dawproject_status))return 1;
    auto& result=*job->result;out->status=result.status.load(std::memory_order_acquire);
    out->revision=result.revision;out->completed_entries=result.completedEntries.load(std::memory_order_acquire);
    out->total_entries=result.totalEntries;out->warning_count=result.warningCount;out->info_count=result.infoCount;
    std::memset(out->error,0,sizeof(out->error));if(out->status==2)std::memcpy(out->error,result.error,sizeof(out->error));return 0;
}
...
int daw_get_clip(daw_session* s,uint64_t id,uint32_t index,daw_clip* out) { return guard(s,[&]{
    ...
    for(const auto& t:s->model.state().tracks) if(t.id==id) { if(index>=t.regions.size()) throw daw::Error("Clip index out of range"); const auto& r=t.regions[index]; *out={sizeof(daw_clip),r.start,r.sourceOffset,r.length,r.fadeIn,r.fadeOut,r.take,r.color,r.gain,r.muted,r.looped,r.pan}; return; }
    throw daw::Error("Track not found");
}); }
int daw_edit_clip(daw_session* s,uint64_t id,uint32_t index,uint64_t start,uint64_t offset,uint64_t length,uint64_t rev) {
    return guard(s,[&]{s->model.editClip(id,index,start,offset,length,rev); resetTransport(s);});
}
int daw_edit_clip_full(daw_session* s,uint64_t id,uint32_t index,uint64_t start,uint64_t offset,uint64_t length,uint64_t fadeIn,uint64_t fadeOut,uint64_t rev) {
    return guard(s,[&]{s->model.editClipFull(id,index,start,offset,length,fadeIn,fadeOut,rev); resetTransport(s);});
}
int daw_split_clip(daw_session* s,uint64_t id,uint32_t index,uint64_t frame,uint64_t rev) {
    return guard(s,[&]{s->model.splitClip(id,index,frame,rev); resetTransport(s);});
}
int daw_duplicate_clip(daw_session* s,uint64_t id,uint32_t index,uint64_t rev) { return guard(s,[&]{s->model.duplicateClip(id,index,rev); resetTransport(s);}); }
...
int daw_set_clip_pan(daw_session* s,uint64_t id,uint32_t index,double pan,uint64_t rev){return guard(s,[&]{s->model.setClipPan(id,index,pan,rev);resetTransport(s);});}
int daw_seek_frame(daw_session* s,uint64_t frame) { return guard(s,[&]{
    ...
    invalidatePlaybackPreparation(s);s->output.reset(); s->selectedFrame=frame;
}); }
int daw_set_loop(daw_session* s,int32_t enabled,uint64_t start,uint64_t end){return guard(s,[&]{
    if(recordingActive(s))throw daw::Error("Stop recording before changing the loop");
    if(enabled!=0&&enabled!=1)throw daw::Error("Loop enabled must be 0 or 1");
    if(enabled&&(end<=start||end>duration(s)))throw daw::Error("Loop range must be inside the project");
    invalidatePlaybackPreparation(s);if(s->output){s->output->stop();s->selectedFrame=s->output->renderer.audiblePositionFrames();s->output.reset();}
    s->loopEnabled=enabled!=0;s->loopStart=s->loopEnabled?start:0;s->loopEnd=s->loopEnabled?end:0;
    if(s->loopEnabled&&s->selectedFrame>=s->loopEnd)s->selectedFrame=s->loopStart;
});}
int daw_get_waveform(daw_session* s,uint64_t id,float* peaks,uint32_t count) { return guard(s,[&]{
    if(!peaks || count!=512) throw daw::Error("Waveform requires a 512-float buffer");
    for(const auto& t:s->model.state().tracks) if(t.id==id) {
        if(t.audio) std::copy(t.audio->peaks().begin(),t.audio->peaks().end(),peaks);
        else std::fill_n(peaks,count,0);
        return;
    }
    throw daw::Error("Track not found");
}); }
int daw_play(daw_session* s) { return guard(s,[&]{
    if(recordingActive(s)) throw daw::Error("Stop recording before playback");
    if(s->playbackPreparation)return;
    if(s->output&&s->output->renderer.playing.load(std::memory_order_acquire))return;
    beginPlaybackPreparation(s);
}); }
int daw_record_start(daw_session* s,uint64_t startFrame,const char* recoveryPath) { return guard(s,[&]{
    startRecording(s,startFrame,recoveryPath,0);
}); }
int daw_record_start_take(daw_session* s,uint64_t id,uint64_t startFrame,const char* recoveryPath){return guard(s,[&]{startRecording(s,startFrame,recoveryPath,id);});}
int daw_record_stop(daw_session* s,const char* name,uint64_t rev) { return guard(s,[&]{
    if(!recordingActive(s)) throw daw::Error("Recording is not active");
    std::string takeName=required(name); daw::validateName(takeName);
...
int daw_get_recording(daw_session* s,daw_recording* out) { return guard(s,[&]{
    if(!out || out->struct_size!=sizeof(daw_recording)) throw daw::Error("Recording ABI mismatch");
    *out={sizeof(daw_recording),0,0,0,0,0,0,0}; if(!recordingActive(s)) return;
    if(s->duplex){s->duplex->checkDevices();out->overflowed=s->duplex->overflowed();out->frames=s->duplex->frames();out->callbacks=s->duplex->callbacks();out->loop_recording=1;const auto loopFrames=s->loopEnd-s->loopStart;out->pass_count=static_cast<uint32_t>((out->frames+loopFrames-1)/loopFrames);}else{s->input->checkDevice();out->overflowed=s->input->overflowed();out->frames=s->input->frames();out->callbacks=s->input->callbacks();}
    out->recording=1;out->target_track_id=s->recordTarget;
...
int daw_recover_take(daw_session* s,const char* path,const char* name,uint64_t rev) { return guard(s,[&]{
    ...
    auto recovered=daw::recoverTake(recoveryPath);s->model.importAt(takeName,std::move(recovered.clip),recovered.startFrame,rev);
    std::error_code error;std::filesystem::remove(recoveryPath,error);
    s->selectedFrame=recovered.startFrame;resetTransport(s);
}); }
int daw_stop(daw_session* s) { return guard(s,[&]{if(s->duplex)throw daw::Error("Use recording stop while loop recording is active");invalidatePlaybackPreparation(s);s->transientOutputState=0;if(s->output) { s->output->stop(); s->selectedFrame=s->output->renderer.audiblePositionFrames();s->output->renderer.resetMeters(); }}); }
int daw_get_transport(daw_session* s,daw_transport* out) { return guard(s,[&]{
    if(!out || out->struct_size!=sizeof(daw_transport)) throw daw::Error("Transport ABI mismatch");
    pollPlaybackPreparation(s);*out={sizeof(daw_transport),0,s->selectedFrame,duration(s),0,0,0,s->loopEnabled?1:0,s->loopStart,s->loopEnd,0,0}; if(!s->output&&!s->duplex) return;
    auto& r=s->duplex?s->duplex->renderer:s->output->renderer;if(s->duplex)s->duplex->checkDevices();else s->output->checkDevice();
    auto now=std::chrono::steady_clock::now(); auto callbacks=r.callbacks.load();
    if(callbacks!=s->lastCallbacks) { s->lastCallbacks=callbacks; s->lastProgress=now; }
    if(r.playing.load() && now-s->lastProgress>std::chrono::seconds(2)) { if(s->duplex)s->duplex->markStalled();else s->output->markStalled();throw daw::Error("Output stalled: no audio callbacks for 2 seconds"); }
    out->playing=r.playing.load(); out->frame=r.audiblePositionFrames(); s->selectedFrame=out->frame; out->duration=r.duration(); out->callbacks=callbacks; out->clipped_frames=r.clipped.load(); out->peak=r.peak.load();out->plugin_errors=r.pluginErrors.load();out->output_latency_frames=r.masterLatencyFrames();
    if(!out->playing&&s->output) s->output->stop();
}); }
int daw_get_channel_meter(daw_session* s,int32_t owner,uint64_t ownerID,daw_channel_meter* out){return guard(s,[&]{
    if(!out||out->struct_size!=sizeof(daw_channel_meter))throw daw::Error("Channel meter ABI mismatch");
    *out={sizeof(daw_channel_meter),DAW_CHANNEL_METER_VERSION,0,0};
    ...
    const auto& state=s->model.state();validateInsertOwner(owner,ownerID);
    switch(owner){
        case DAW_INSERT_OWNER_TRACK:if(std::none_of(state.tracks.begin(),state.tracks.end(),[&](const auto& item){return item.id==ownerID;}))throw daw::Error("Track not found");break;
        case DAW_INSERT_OWNER_BUS:if(std::none_of(state.buses.begin(),state.buses.end(),[&](const auto& item){return item.id==ownerID;}))throw daw::Error("Bus not found");break;
        case DAW_INSERT_OWNER_MASTER:break;
        default:throw daw::Error("Unsupported insert owner");
    }
...
int daw_get_output_status(daw_session* s,daw_output_status* out){return guard(s,[&]{
    ...
    *out={sizeof(daw_output_status),s->transientOutputState,0,s->playbackGeneration,0,0};if(s->playbackPreparation)return;if(!s->output&&!s->duplex)return;const auto status=s->duplex?s->duplex->telemetry():s->output->telemetry();
    out->state=static_cast<int32_t>(status.state);out->device_id=status.deviceID;out->generation=status.generation;out->callbacks=status.callbacks;out->callback_errors=status.callbackErrors;
});}
void daw_error(daw_session* s,char* buffer,size_t capacity) {
    if (buffer && capacity) std::snprintf(buffer,capacity,"%s",s?s->error:"Missing session");
}
}
