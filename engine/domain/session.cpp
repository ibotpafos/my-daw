...
void validateName(const std::string& name) {
    ...
    for (size_t i = 0; i < name.size();) {
        auto c = static_cast<unsigned char>(name[i++]); uint32_t cp; int more;
        if (c < 0x80) { cp = c; more = 0; }
        else if (c >= 0xC2 && c <= 0xDF) { cp = c & 31; more = 1; }
        else if (c >= 0xE0 && c <= 0xEF) { cp = c & 15; more = 2; }
        else if (c >= 0xF0 && c <= 0xF4) { cp = c & 7; more = 3; }
        else throw Error("Invalid UTF-8 name");
        for (int j = 0; j < more; ++j) {
            if (i >= name.size()) throw Error("Truncated UTF-8 name");
            auto b = static_cast<unsigned char>(name[i++]);
            if ((b & 0xC0) != 0x80) throw Error("Invalid UTF-8 continuation");
            cp = (cp << 6) | (b & 63);
        }
    ...
    if (count == 0 || count > 120) throw Error("Name must contain 1–120 characters");
}
namespace {
void validateAutomation(const std::vector<AutomationPoint>& points,double minimum,double maximum,const char* label){
    if(points.size()>2048)throw Error(std::string(label)+" supports at most 2048 automation points");
    uint64_t previousFrame=0;bool first=true;
    for(const auto& point:points){
        if(point.frame>48000*600)throw Error("Automation point exceeds timeline limit");
        if(!std::isfinite(point.gainDb)||point.gainDb<minimum||point.gainDb>maximum)throw Error(std::string(label)+" automation value is outside its range");
        if(!first&&point.frame<=previousFrame)throw Error("Automation frames must be strictly ordered");
        previousFrame=point.frame;first=false;
    }
}
void upsertAutomation(std::vector<AutomationPoint>& points,uint64_t frame,double value){
    ...
    if(point!=points.end()&&point->frame==frame)point->gainDb=value;else points.insert(point,{frame,value});
}
...
std::vector<AutomationPoint>& automationLane(State& state,AutomationTarget target,uint64_t targetID){
    ...
    case AutomationTarget::TrackPan:{auto track=std::find_if(state.tracks.begin(),state.tracks.end(),[&](const auto& item){return item.id==targetID;});if(track==state.tracks.end())throw Error("Track not found");return track->panAutomation;}
    case AutomationTarget::BusGain:{auto bus=std::find_if(state.buses.begin(),state.buses.end(),[&](const auto& item){return item.id==targetID;});if(bus==state.buses.end())throw Error("Bus not found");return bus->gainAutomation;}
    case AutomationTarget::MasterGain:if(targetID!=0)throw Error("Master automation target ID must be zero");return state.masterGainAutomation;
    }
    throw Error("Unsupported automation target");
}
void validateGestureValue(AutomationTarget target,uint64_t frame,double value){
    if(frame>48000*600)throw Error("Automation point exceeds timeline limit");
    const auto pan=target==AutomationTarget::TrackPan;
    if(!std::isfinite(value)||value<(pan?-1.0:-120.0)||value>(pan?1.0:24.0))throw Error("Automation value is outside its range");
}
void applyWorkflow(State& state,const std::vector<WorkflowOperation>& batch){
    if(batch.empty()||batch.size()>100)throw Error("Workflow supports 1–100 operations");
    for(const auto& operation:batch){
        auto track=std::find_if(state.tracks.begin(),state.tracks.end(),[&](const auto& item){return item.id==operation.trackID;});
        if(track==state.tracks.end())throw Error("Workflow track not found");
        switch(operation.kind){
        case WorkflowOperationKind::RenameTrack: track->name=operation.name;break;
        case WorkflowOperationKind::SetTrackGain: track->gain=operation.gainDb;break;
        default: throw Error("Unsupported workflow operation");
        }
    }
}
std::vector<WorkflowTrackSummary> workflowSummary(const State& state){
    std::vector<WorkflowTrackSummary> result;result.reserve(state.tracks.size());
    for(const auto& track:state.tracks)result.push_back({track.id,track.name,track.gain});
    return result;
}
}
void validate(const State& state) {
    ...
    for (const auto& t : state.tracks) {
        validateName(t.name);
        if(t.audio) {
            ++audioCount;++audioAssets; audioBytes+=t.audio->samples().size()*sizeof(float);
            if(t.baseStart>48000*600||t.audio->frames()>48000*600-t.baseStart)throw Error("Invalid base take bounds");
            if(t.takes.size()>15)throw Error("Track supports at most 16 takes");
            for(const auto& take:t.takes){validateName(take.name);if(!take.audio||take.start>48000*600||take.audio->frames()>48000*600-take.start)throw Error("Invalid take bounds");++audioAssets;audioBytes+=take.audio->samples().size()*sizeof(float);}
            if(t.regions.empty() || t.regions.size()>256) throw Error("Audio track must contain 1–256 clips");
    ...
    for(const auto& bus:state.buses){
        ...
        if(bus.id==0||bus.id>=state.nextID||!ids.insert(bus.id).second||!busIDs.insert(bus.id).second)throw Error("Invalid bus ID");
        if(!std::isfinite(bus.gain)||bus.gain < -120||bus.gain > 24)throw Error("Bus gain outside -120…24 dB");
        if(!std::isfinite(bus.pan)||bus.pan < -1||bus.pan > 1)throw Error("Bus pan outside -1…1");
        validateAutomation(bus.gainAutomation,-120,24,"Bus gain");
    }
    auto hasBus=[&](uint64_t id){return id==0||busIDs.contains(id);};
    ...
    auto busIndex=[&](uint64_t id){return static_cast<size_t>(std::find_if(state.buses.begin(),state.buses.end(),[&](const auto& bus){return bus.id==id;})-state.buses.begin());};
    auto visit=[&](auto&& self,size_t index)->void{if(colours[index]==1)throw Error("Bus routing cycle");if(colours[index]==2)return;colours[index]=1;const auto output=state.buses[index].outputBus;if(output)self(self,busIndex(output));colours[index]=2;};
    for(size_t i=0;i<state.buses.size();++i)visit(visit,i);
    ...
    const auto validateInserts=[&](const std::vector<PluginInsert>& inserts,const char* owner){
        if(inserts.size()>kMaxPluginInsertsPerOwner)throw Error(std::string(owner)+" supports at most 4 inserts");
        if(pluginCount>kMaxProjectPluginInserts-inserts.size())throw Error("Project supports at most 64 plug-in inserts");
        pluginCount+=inserts.size();
        for(const auto& plugin:inserts){
        validateName(plugin.name);
        if(plugin.id==0||plugin.id>=state.nextID||!ids.insert(plugin.id).second)throw Error("Invalid plugin ID");
        if(plugin.hostingMode!=PluginHostingMode::InProcess&&plugin.hostingMode!=PluginHostingMode::OutOfProcess)throw Error("Invalid plug-in hosting mode");
        const auto vst3=isVst3PluginInsert(plugin);
        if(!vst3&&(!plugin.type||!plugin.subtype||!plugin.manufacturer))throw Error("Invalid Audio Unit component ID");
        if(vst3){
            std::string envelopeError;
            if(!decodeVst3StateEnvelope(plugin.state,&envelopeError))throw Error("Invalid VST3 state envelope: "+envelopeError);
        }
        if(plugin.latencyFrames>48000*10)throw Error("Plugin latency exceeds 10 seconds");
        const auto perPluginLimit=vst3?kMaxVst3PluginStateBytes:kMaxAudioUnitPluginStateBytes;
        if(plugin.state.size()>perPluginLimit||pluginStateBytes>kMaxProjectPluginStateBytes-plugin.state.size())throw Error("Plugin state exceeds project limit");
        pluginStateBytes+=plugin.state.size();
        if(plugin.parameterAutomation.size()>kMaxPluginParameterAutomationLanes)throw Error("Plug-in supports at most 128 parameter automation lanes");
        std::set<uint32_t> parameterIDs;
        for(const auto& lane:plugin.parameterAutomation){
            if(!parameterIDs.insert(lane.parameterID).second)throw Error("Duplicate plug-in parameter automation lane");
            if(!lane.name.empty())validateName(lane.name);
            if(lane.points.size()>kMaxPluginParameterAutomationPoints||pluginAutomationPoints>kMaxProjectPluginParameterAutomationPoints-lane.points.size())throw Error("Plug-in parameter automation exceeds project limit");
            pluginAutomationPoints+=lane.points.size();uint64_t previousFrame=0;bool first=true;
            for(const auto& point:lane.points){if(point.frame>48000*600||!std::isfinite(point.normalizedValue)||point.normalizedValue<0||point.normalizedValue>1)throw Error("Invalid plug-in parameter automation point");if(!first&&point.frame<=previousFrame)throw Error("Plug-in parameter automation frames must be strictly ordered");previousFrame=point.frame;first=false;}
        }
        }
    };
    for(const auto& track:state.tracks)validateInserts(track.inserts,"Track");
    for(const auto& bus:state.buses)validateInserts(bus.inserts,"Bus");
    validateInserts(state.masterInserts,"Master");
}
std::vector<WorkflowOperation> prepareVocalTracks(const State& state,const std::vector<uint64_t>& selectedTrackIDs,const std::string& namingBase,const std::vector<VocalGainSuggestion>& suggestions){
    if(selectedTrackIDs.empty()||selectedTrackIDs.size()>32)throw Error("Vocal workflow supports 1–32 selected tracks");
    validateName(namingBase);
    std::set<uint64_t> selected;
    for(const auto id:selectedTrackIDs){
        if(!selected.insert(id).second)throw Error("Vocal workflow selection contains duplicate tracks");
        if(std::find_if(state.tracks.begin(),state.tracks.end(),[&](const auto& track){return track.id==id;})==state.tracks.end())throw Error("Vocal workflow track not found");
    }
    if(suggestions.size()>selectedTrackIDs.size())throw Error("Too many vocal gain suggestions");
    std::vector<VocalGainSuggestion> normalizedSuggestions;normalizedSuggestions.reserve(suggestions.size());std::set<uint64_t> suggested;
    for(const auto& suggestion:suggestions){
        if(!selected.contains(suggestion.trackID))throw Error("Vocal gain suggestion track is not selected");
        if(!suggested.insert(suggestion.trackID).second)throw Error("Duplicate vocal gain suggestion");
        if(!std::isfinite(suggestion.suggestedGainDb))throw Error("Vocal gain suggestion must be finite");
        normalizedSuggestions.push_back({suggestion.trackID,std::clamp(suggestion.suggestedGainDb,-120.0,24.0)});
    }
    std::vector<WorkflowOperation> batch;batch.reserve(selectedTrackIDs.size()+normalizedSuggestions.size());
    for(size_t index=0;index<selectedTrackIDs.size();++index){
        const auto suffix=index==0?std::string(" Lead"):std::string(" Double ")+std::to_string(index);
        const auto name=namingBase+suffix;
        validateName(name);batch.push_back({WorkflowOperationKind::RenameTrack,selectedTrackIDs[index],name,0});
        const auto suggestion=std::find_if(normalizedSuggestions.begin(),normalizedSuggestions.end(),[&](const auto& item){return item.trackID==selectedTrackIDs[index];});
        if(suggestion!=normalizedSuggestions.end())batch.push_back({WorkflowOperationKind::SetTrackGain,selectedTrackIDs[index],{},suggestion->suggestedGainDb});
    }
    return batch;
}
...
void Session::commit(State next) {
    next.revision = current.revision + 1; validate(next);
    ...
    auto retainedBytes = [&] {
        std::set<const Clip*> seen; size_t bytes=0;
        auto count=[&](const State& state){ for(const auto& t:state.tracks){if(t.audio&&seen.insert(t.audio.get()).second)bytes+=t.audio->samples().size()*sizeof(float);for(const auto& take:t.takes)if(take.audio&&seen.insert(take.audio.get()).second)bytes+=take.audio->samples().size()*sizeof(float);} };
        count(next); for(const auto& state:history) count(state); return bytes;
    };
    while(!history.empty() && retainedBytes()>128*1024*1024) history.erase(history.begin());
    past.swap(history); future.clear(); current = std::move(next);
}
...
void Session::rename(uint64_t id, const std::string& name, uint64_t expected) {
    ...
    it->name = name; commit(std::move(next));
}
void Session::gain(uint64_t id, double value, uint64_t expected) {
    check(expected); State next = current;
    auto it = std::find_if(next.tracks.begin(), next.tracks.end(), [id](const auto& t){return t.id == id;});
    if (it == next.tracks.end()) throw Error("Track not found");
    if (it->gain == value) return;
    it->gain = value; commit(std::move(next));
}
...
void Session::removeSend(uint64_t trackID,uint64_t busID,uint64_t expected){check(expected);State next=current;auto track=std::find_if(next.tracks.begin(),next.tracks.end(),[&](const auto& item){return item.id==trackID;});if(track==next.tracks.end())throw Error("Track not found");auto send=std::find_if(track->sends.begin(),track->sends.end(),[&](const auto& item){return item.bus==busID;});if(send==track->sends.end())throw Error("Send not found");track->sends.erase(send);commit(std::move(next));}
void Session::upsertTrackVolumeAutomation(uint64_t trackID,uint64_t frame,double gainDb,uint64_t expected){
    ...
    if(point!=track->volumeAutomation.end()&&point->frame==frame){if(point->gainDb==gainDb)return;point->gainDb=gainDb;}else track->volumeAutomation.insert(point,{frame,gainDb});
    commit(std::move(next));
}
...
void Session::upsertTrackPanAutomation(uint64_t trackID,uint64_t frame,double pan,uint64_t expected){
    check(expected);State next=current;auto track=std::find_if(next.tracks.begin(),next.tracks.end(),[&](const auto& item){return item.id==trackID;});if(track==next.tracks.end())throw Error("Track not found");
    const auto before=track->panAutomation;upsertAutomation(track->panAutomation,frame,pan);if(track->panAutomation==before)return;commit(std::move(next));
}
...
void Session::upsertBusGainAutomation(uint64_t busID,uint64_t frame,double gainDb,uint64_t expected){
    ...
    const auto before=bus->gainAutomation;upsertAutomation(bus->gainAutomation,frame,gainDb);if(bus->gainAutomation==before)return;commit(std::move(next));
}
...
void Session::removeMasterGainAutomation(uint64_t frame,uint64_t expected){
    check(expected);State next=current;if(!removeAutomation(next.masterGainAutomation,frame))throw Error("Automation point not found");commit(std::move(next));
}
void Session::beginAutomationGesture(AutomationTarget target,uint64_t targetID,AutomationWriteMode mode,uint64_t expected){
    check(expected);
    if(mode!=AutomationWriteMode::Touch&&mode!=AutomationWriteMode::Latch)throw Error("Unsupported automation write mode");
...
void Session::writeAutomationGesture(uint64_t frame,double value){
    if(!gesture)throw Error("No active automation gesture");
    if(gesture->hasWritten&&frame<gesture->lastFrame)throw Error("Automation gesture frames must be ordered");
    ...
    validate(next);
    gesture->working=std::move(next);gesture->hasWritten=true;gesture->lastFrame=frame;gesture->lastValue=value;
}
void Session::endAutomationGesture(uint64_t endFrame,uint64_t expected){
    if(!gesture)throw Error("No active automation gesture");
    if(expected!=gesture->baseRevision||current.revision!=gesture->baseRevision)throw Error("Revision conflict: refresh the project");
    if(endFrame>48000*600)throw Error("Automation point exceeds timeline limit");
    if(gesture->hasWritten&&endFrame<gesture->lastFrame)throw Error("Automation gesture end precedes its final sample");
    auto finished=std::move(*gesture);gesture.reset();
    if(!finished.hasWritten)return;
    if(finished.mode==AutomationWriteMode::Latch&&endFrame>finished.lastFrame){
        upsertAutomation(automationLane(finished.working,finished.target,finished.targetID),endFrame,finished.lastValue);
    }
    validate(finished.working);
    commit(std::move(finished.working));
}
void Session::cancelAutomationGesture() noexcept{gesture.reset();}
bool Session::automationGestureActive() const noexcept{return gesture.has_value();}
WorkflowPreview Session::previewWorkflow(const std::vector<WorkflowOperation>& batch,uint64_t expected) const{
    check(expected);State next=current;applyWorkflow(next,batch);validate(next);
    return {current.revision,current.revision+(next.tracks==current.tracks?0:1),workflowSummary(current),workflowSummary(next)};
}
void Session::commitWorkflow(const std::vector<WorkflowOperation>& batch,uint64_t expected){
    check(expected);State next=current;applyWorkflow(next,batch);if(next.tracks==current.tracks)return;commit(std::move(next));
}
void Session::addMasterInsert(PluginInsert plugin,uint64_t expected){check(expected);State next=current;if(next.masterInserts.size()>=4)throw Error("Master supports at most 4 inserts");plugin.id=next.nextID++;next.masterInserts.push_back(std::move(plugin));commit(std::move(next));}
void Session::moveMasterInsert(uint64_t id,uint32_t newIndex,uint64_t expected){
    check(expected);State next=current;auto current=std::find_if(next.masterInserts.begin(),next.masterInserts.end(),[&](const auto& plugin){return plugin.id==id;});
    if(current==next.masterInserts.end())throw Error("Master insert not found");
    if(newIndex>=next.masterInserts.size())throw Error("Master insert index out of range");
    const auto oldIndex=static_cast<uint32_t>(current-next.masterInserts.begin());if(oldIndex==newIndex)return;
    if(oldIndex<newIndex)std::rotate(current,current+1,next.masterInserts.begin()+newIndex+1);
    else std::rotate(next.masterInserts.begin()+newIndex,current,current+1);
    commit(std::move(next));
}
void Session::removeMasterInsert(uint64_t id,uint64_t expected){check(expected);State next=current;auto it=std::find_if(next.masterInserts.begin(),next.masterInserts.end(),[&](const auto& plugin){return plugin.id==id;});if(it==next.masterInserts.end())throw Error("Master insert not found");next.masterInserts.erase(it);commit(std::move(next));}
void Session::bypassMasterInsert(uint64_t id,bool bypassed,uint64_t expected){check(expected);State next=current;auto it=std::find_if(next.masterInserts.begin(),next.masterInserts.end(),[&](const auto& plugin){return plugin.id==id;});if(it==next.masterInserts.end())throw Error("Master insert not found");if(it->bypassed==bypassed)return;it->bypassed=bypassed;commit(std::move(next));}
void Session::setMasterInsertHostingMode(uint64_t id,PluginHostingMode mode,uint64_t expected){check(expected);State next=current;auto plugin=std::find_if(next.masterInserts.begin(),next.masterInserts.end(),[&](const auto& item){return item.id==id;});if(plugin==next.masterInserts.end())throw Error("Master insert not found");if(plugin->hostingMode==mode)return;plugin->hostingMode=mode;validate(next);commit(std::move(next));}
void Session::updateMasterInsertState(uint64_t id,std::vector<uint8_t> state,uint32_t latencyFrames,uint64_t expected){
    check(expected);State next=current;auto it=std::find_if(next.masterInserts.begin(),next.masterInserts.end(),[&](const auto& plugin){return plugin.id==id;});if(it==next.masterInserts.end())throw Error("Master insert not found");
...
std::vector<PluginInsert>& busInserts(State& state,uint64_t busID){auto bus=std::find_if(state.buses.begin(),state.buses.end(),[&](const auto& item){return item.id==busID;});if(bus==state.buses.end())throw Error("Bus not found");return bus->inserts;}
std::vector<PluginInsert>& ownerInserts(State& state,PluginOwner owner,uint64_t ownerID){switch(owner){case PluginOwner::Track:return trackInserts(state,ownerID);case PluginOwner::Bus:return busInserts(state,ownerID);case PluginOwner::Master:if(ownerID)throw Error("Master plug-in owner ID must be zero");return state.masterInserts;}throw Error("Unsupported plug-in owner");}
PluginInsert& ownerPlugin(State& state,PluginOwner owner,uint64_t ownerID,uint64_t pluginID){auto& inserts=ownerInserts(state,owner,ownerID);auto plugin=std::find_if(inserts.begin(),inserts.end(),[&](const auto& item){return item.id==pluginID;});if(plugin==inserts.end())throw Error("Plug-in insert not found");return *plugin;}
bool moveInsert(std::vector<PluginInsert>& inserts,uint64_t id,uint32_t newIndex){auto current=std::find_if(inserts.begin(),inserts.end(),[&](const auto& plugin){return plugin.id==id;});if(current==inserts.end())throw Error("Insert not found");if(newIndex>=inserts.size())throw Error("Insert index out of range");const auto oldIndex=static_cast<uint32_t>(current-inserts.begin());if(oldIndex==newIndex)return false;if(oldIndex<newIndex)std::rotate(current,current+1,inserts.begin()+newIndex+1);else std::rotate(inserts.begin()+newIndex,current,current+1);return true;}
bool updateInsertState(std::vector<PluginInsert>& inserts,uint64_t id,std::vector<uint8_t> state,uint32_t latencyFrames){auto it=std::find_if(inserts.begin(),inserts.end(),[&](const auto& plugin){return plugin.id==id;});if(it==inserts.end())throw Error("Insert not found");if(isVst3PluginInsert(*it)){std::string envelopeError;if(!decodeVst3StateEnvelope(state,&envelopeError))throw Error("Invalid VST3 state envelope: "+envelopeError);}else if(state.size()>=4&&state[0]=='M'&&state[1]=='D'&&state[2]=='V'&&state[3]=='S')throw Error("Audio Unit state cannot be replaced with a VST3 envelope");if(it->state==state&&it->latencyFrames==latencyFrames)return false;it->state=std::move(state);it->latencyFrames=latencyFrames;return true;}
bool bypassInsert(std::vector<PluginInsert>& inserts,uint64_t id,bool bypassed){auto it=std::find_if(inserts.begin(),inserts.end(),[&](const auto& plugin){return plugin.id==id;});if(it==inserts.end())throw Error("Insert not found");if(it->bypassed==bypassed)return false;it->bypassed=bypassed;return true;}
bool setInsertHostingMode(std::vector<PluginInsert>& inserts,uint64_t id,PluginHostingMode mode){auto it=std::find_if(inserts.begin(),inserts.end(),[&](const auto& plugin){return plugin.id==id;});if(it==inserts.end())throw Error("Insert not found");if(it->hostingMode==mode)return false;it->hostingMode=mode;return true;}
}
void Session::addTrackInsert(uint64_t trackID,PluginInsert plugin,uint64_t expected){check(expected);State next=current;auto& inserts=trackInserts(next,trackID);if(inserts.size()>=kMaxPluginInsertsPerOwner)throw Error("Track supports at most 4 inserts");plugin.id=next.nextID++;inserts.push_back(std::move(plugin));commit(std::move(next));}
void Session::moveTrackInsert(uint64_t trackID,uint64_t id,uint32_t newIndex,uint64_t expected){check(expected);State next=current;if(!moveInsert(trackInserts(next,trackID),id,newIndex))return;commit(std::move(next));}
void Session::removeTrackInsert(uint64_t trackID,uint64_t id,uint64_t expected){check(expected);State next=current;auto& inserts=trackInserts(next,trackID);auto it=std::find_if(inserts.begin(),inserts.end(),[&](const auto& plugin){return plugin.id==id;});if(it==inserts.end())throw Error("Track insert not found");inserts.erase(it);commit(std::move(next));}
void Session::bypassTrackInsert(uint64_t trackID,uint64_t id,bool bypassed,uint64_t expected){check(expected);State next=current;if(!bypassInsert(trackInserts(next,trackID),id,bypassed))return;commit(std::move(next));}
void Session::setTrackInsertHostingMode(uint64_t trackID,uint64_t id,PluginHostingMode mode,uint64_t expected){check(expected);State next=current;if(!setInsertHostingMode(trackInserts(next,trackID),id,mode))return;validate(next);commit(std::move(next));}
void Session::updateTrackInsertState(uint64_t trackID,uint64_t id,std::vector<uint8_t> state,uint32_t latencyFrames,uint64_t expected){check(expected);State next=current;if(!updateInsertState(trackInserts(next,trackID),id,std::move(state),latencyFrames))return;commit(std::move(next));}
void Session::addBusInsert(uint64_t busID,PluginInsert plugin,uint64_t expected){check(expected);State next=current;auto& inserts=busInserts(next,busID);if(inserts.size()>=kMaxPluginInsertsPerOwner)throw Error("Bus supports at most 4 inserts");plugin.id=next.nextID++;inserts.push_back(std::move(plugin));commit(std::move(next));}
void Session::moveBusInsert(uint64_t busID,uint64_t id,uint32_t newIndex,uint64_t expected){check(expected);State next=current;if(!moveInsert(busInserts(next,busID),id,newIndex))return;commit(std::move(next));}
void Session::removeBusInsert(uint64_t busID,uint64_t id,uint64_t expected){check(expected);State next=current;auto& inserts=busInserts(next,busID);auto it=std::find_if(inserts.begin(),inserts.end(),[&](const auto& plugin){return plugin.id==id;});if(it==inserts.end())throw Error("Bus insert not found");inserts.erase(it);commit(std::move(next));}
void Session::bypassBusInsert(uint64_t busID,uint64_t id,bool bypassed,uint64_t expected){check(expected);State next=current;if(!bypassInsert(busInserts(next,busID),id,bypassed))return;commit(std::move(next));}
void Session::setBusInsertHostingMode(uint64_t busID,uint64_t id,PluginHostingMode mode,uint64_t expected){check(expected);State next=current;if(!setInsertHostingMode(busInserts(next,busID),id,mode))return;validate(next);commit(std::move(next));}
void Session::updateBusInsertState(uint64_t busID,uint64_t id,std::vector<uint8_t> state,uint32_t latencyFrames,uint64_t expected){check(expected);State next=current;if(!updateInsertState(busInserts(next,busID),id,std::move(state),latencyFrames))return;commit(std::move(next));}
void Session::upsertPluginParameterAutomation(PluginOwner owner,uint64_t ownerID,uint64_t pluginID,uint32_t parameterID,const std::string& name,uint64_t frame,double normalizedValue,uint64_t expected){
    check(expected);if(frame>48000*600||!std::isfinite(normalizedValue)||normalizedValue<0||normalizedValue>1)throw Error("Invalid plug-in parameter automation point");if(!name.empty())validateName(name);
    State next=current;auto& plugin=ownerPlugin(next,owner,ownerID,pluginID);auto lane=std::find_if(plugin.parameterAutomation.begin(),plugin.parameterAutomation.end(),[&](const auto& item){return item.parameterID==parameterID;});bool changed=false;
    if(lane==plugin.parameterAutomation.end()){if(plugin.parameterAutomation.size()>=kMaxPluginParameterAutomationLanes)throw Error("Plug-in supports at most 128 parameter automation lanes");plugin.parameterAutomation.push_back({parameterID,name,{}});lane=std::prev(plugin.parameterAutomation.end());changed=true;}
    else if(!name.empty()&&lane->name!=name){lane->name=name;changed=true;}
...
void Session::beginPluginParameterAutomationGesture(PluginOwner owner,uint64_t ownerID,uint64_t pluginID,uint32_t parameterID,const std::string& name,AutomationWriteMode mode,uint64_t expected){
    check(expected);if(mode!=AutomationWriteMode::Touch&&mode!=AutomationWriteMode::Latch)throw Error("Unsupported automation write mode");if(!name.empty())validateName(name);
    State working=current;(void)ownerPlugin(working,owner,ownerID,pluginID);
...
uint64_t takeStart(const Track& t,uint32_t take){return take==0?t.baseStart:t.takes[take-1].start;} }
void Session::copyClipToTrack(uint64_t sourceTrack,uint32_t index,uint64_t targetTrack,uint64_t start,uint64_t expected) {
    check(expected); State next=current;
    auto src=std::find_if(next.tracks.begin(),next.tracks.end(),[sourceTrack](const auto& t){return t.id==sourceTrack;});
    if(src==next.tracks.end()||!src->audio||index>=src->regions.size()) throw Error("Audio clip not found");
...
void Session::moveClipToTrack(uint64_t sourceTrack,uint32_t index,uint64_t targetTrack,uint64_t start,uint64_t expected) {
    ...
    if(!pasted||pasted!=takeAudio(*src,source.take)||takeStart(*dst,source.take)!=takeStart(*src,source.take)) throw Error("The target track does not share the clip's audio source");
    if(start>48000*600||source.length>48000*600-start) throw Error("No timeline space for the clip");
    if(src!=dst&&src->regions.size()==1) throw Error("The last clip keeps the imported audio attached to its track");
...
void Session::copyMidiClipToTrack(uint64_t sourceTrack,uint32_t index,uint64_t targetTrack,uint64_t start,uint64_t expected) {
    check(expected); State next=current;
    auto src=std::find_if(next.tracks.begin(),next.tracks.end(),[sourceTrack](const auto& t){return t.id==sourceTrack;});
    if(src==next.tracks.end()||index>=src->midiClips.size()) throw Error("MIDI clip not found");
    auto dst=std::find_if(next.tracks.begin(),next.tracks.end(),[targetTrack](const auto& t){return t.id==targetTrack;});
    if(dst==next.tracks.end()) throw Error("Track not found");
    auto copy=src->midiClips[index]; copy.start=start;
    dst->midiClips.push_back(std::move(copy)); commit(std::move(next));
}
void Session::moveMidiClipToTrack(uint64_t sourceTrack,uint32_t index,uint64_t targetTrack,uint64_t start,uint64_t expected) {
    check(expected); State next=current;
    auto src=std::find_if(next.tracks.begin(),next.tracks.end(),[sourceTrack](const auto& t){return t.id==sourceTrack;});
    if(src==next.tracks.end()||index>=src->midiClips.size()) throw Error("MIDI clip not found");
    auto dst=std::find_if(next.tracks.begin(),next.tracks.end(),[targetTrack](const auto& t){return t.id==targetTrack;});
...
void Session::undo(uint64_t expected) {
    ...
    State next = past.back(); next.revision = current.revision + 1;
    next.nextID = current.nextID; validate(next);
    future.push_back(current); past.pop_back(); current = std::move(next);
}
void Session::redo(uint64_t expected) {
    check(expected); if (future.empty()) throw Error("Nothing to redo");
