...
class MacDuplex final:public Duplex {
    ...
    static OSStatus callback(void* ref,AudioUnitRenderActionFlags* flags,const AudioTimeStamp* time,UInt32,UInt32 frames,AudioBufferList* output) noexcept {
        ...
        if (!time) { silence(output); self.callbackErrors.fetch_add(1, std::memory_order_relaxed); return kAudio_ParamError; }
        ...
        if (time) {
            if (time->mFlags & kAudioTimeStampSampleTimeValid) {
                stamp.flags |= CaptureTimestamp::sampleValid; stamp.sampleTime = time->mSampleTime;
            }
            if (time->mFlags & kAudioTimeStampHostTimeValid) {
                stamp.flags |= CaptureTimestamp::hostValid; stamp.hostTime = time->mHostTime;
            }
        }
        if (!self.capture->processTimed(stamp, self.inputScratch.data(),
            static_cast<float*>(output->mBuffers[0].mData), static_cast<float*>(output->mBuffers[1].mData), frames)) {
            silence(output); self.callbackErrors.fetch_add(1, std::memory_order_relaxed);
...
public:
    ...
    std::shared_ptr<const Clip> stop() override {
        ...
        // Stop can win the race against the next UI poll. A failed callback
        // must not become a successful project commit on that route either.
        if(callbackErrors.load(std::memory_order_acquire)) {
            capture->cancel();
            throw Error("Recording callback/timestamp failed; the confirmed prefix remains recoverable");
        }
    ...
    void checkDevices() override {if(!active)return;
        if(capture && capture->clockReport().fault != CaptureClockFault::none) {
            shutdown(OutputState::callbackError);
            throw Error("Recording clock discontinuity: input timestamp jumped or repeated; captured audio remains recoverable");
    ...
    CaptureClockReport clockReport() const noexcept override { return capture ? capture->clockReport() : CaptureClockReport{}; }
