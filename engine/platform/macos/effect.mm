...
void configure(Unit &unit, uint32_t componentType, uint32_t sampleRate,
               uint32_t maxFrames, const std::vector<uint8_t> &state) {
  auto stream = format(sampleRate);
  // A MusicDevice is a generator. Configuring an input bus on synths such as
  // DLS/Sampler is invalid and used to be the reason they could not pass the
  // same host path as effects.
...
std::vector<ParameterRange> readParameterRanges(AudioUnit unit) {
  UInt32 listSize = 0;
  ...
  if (!ids.empty())
    checked(AudioUnitGetProperty(unit, kAudioUnitProperty_ParameterList,
  ...
  for (const auto id : ids) {
    ...
    if (AudioUnitGetProperty(unit, kAudioUnitProperty_ParameterInfo,
      ...
      result.push_back({id, info.minValue, info.maxValue});
  }
...
bool scheduleParameters(AudioUnit unit,
                        const std::vector<ParameterRange> &parameters,
                        std::vector<AudioUnitParameterEvent> &scheduled,
                        std::span<const PreparedParameterEvent> events) noexcept {
  size_t scheduledCount = 0;
  for (size_t index = 0; index < events.size(); ++index) {
    const auto &event = events[index];
    if (!std::isfinite(event.normalizedValue) || event.normalizedValue < 0 ||
    ...
    const bool ramp = index + 1 < events.size() &&
                      events[index + 1].parameterID == event.parameterID &&
                      events[index + 1].sampleOffset > event.sampleOffset;
    if (ramp) {
      ...
      scheduledEvent.eventValues.ramp.startValue = native(event.normalizedValue);
      ...
      scheduledEvent.eventValues.immediate.value = native(event.normalizedValue);
    }
  }
  return scheduledCount == 0 ||
         AudioUnitScheduleParameters(unit, scheduled.data(),
                                     static_cast<UInt32>(scheduledCount)) == noErr;
}

uint32_t readLatency(AudioUnit unit, uint32_t sampleRate) {
  Float64 seconds = 0;
  UInt32 size = sizeof(seconds);
  checked(AudioUnitGetProperty(unit, kAudioUnitProperty_Latency,
                               kAudioUnitScope_Global, 0, &seconds, &size),
          "Read initialized Audio Unit latency");
  if (!std::isfinite(seconds) || seconds < 0 || seconds > 10)
    throw Error("Invalid initialized Audio Unit latency");
  return static_cast<uint32_t>(std::llround(seconds * sampleRate));
}
...
public:
  AppleEffect(const PluginInsert &plugin, uint32_t sampleRate,
              uint32_t maxFrames)
      : unit(find({plugin.type, plugin.subtype, plugin.manufacturer, plugin.name}),
             plugin.hostingMode),
        maximum(maxFrames), inputLeft(maxFrames), inputRight(maxFrames) {
    configure(unit, plugin.type, sampleRate, maxFrames, plugin.state);
    ...
    latency = readLatency(unit.value, sampleRate);
    parameters = readParameterRanges(unit.value);
    size_t capacity = 0;
  ...
  bool process(float *left, float *right, uint32_t frames, uint64_t sampleTime,
    ...
    // MIDI belongs to the instrument insert in the same serial chain. Effects
    // must pass it by rather than treating the presence of a track MIDI lane as
    // a processing failure; otherwise an AU/VST effect after a synth would make
    // every instrument block fall back dry.
    (void)midiEvents;
    ...
    if (!scheduleParameters(unit.value, parameters, scheduled, parameterEvents)) {
    ...
    const auto status = AudioUnitRender(
        unit.value, &flags, &time, 0, frames,
...
class AppleMusicDevice final : public PreparedEffect {
  Unit unit;
  uint32_t maximum, latency = 0;
...
public:
  AppleMusicDevice(const PluginInsert &plugin, uint32_t sampleRate,
                   uint32_t maxFrames)
      : unit(find({plugin.type, plugin.subtype, plugin.manufacturer, plugin.name}),
             plugin.hostingMode),
        maximum(maxFrames), dryLeft(maxFrames), dryRight(maxFrames) {
    configure(unit, plugin.type, sampleRate, maxFrames, plugin.state);
    checked(AudioUnitInitialize(unit.value), "Initialize Audio Unit MusicDevice");
    unit.initialized = true;
    latency = readLatency(unit.value, sampleRate);
    parameters = readParameterRanges(unit.value);
    size_t capacity = 0;
  ...
  bool process(float *left, float *right, uint32_t frames, uint64_t sampleTime,
    ...
    std::copy_n(left, frames, dryLeft.begin());
    std::copy_n(right, frames, dryRight.begin());
    if (!scheduleParameters(unit.value, parameters, scheduled, parameterEvents))
      return failDry(left, right, frames);

    for (const auto &event : midiEvents) {
      const UInt32 status = (event.noteOff ? 0x80u : 0x90u) |
                            static_cast<UInt32>(event.channel & 0x0fu);
      const UInt32 velocity = event.noteOff ? 0u : event.velocity;
      if (MusicDeviceMIDIEvent(unit.value, status, event.pitch, velocity,
                               event.sampleOffset) != noErr)
        return failDry(left, right, frames);
    }
...
AudioUnitSnapshot snapshotAudioUnit(const AudioUnitDescriptor &descriptor,
  ...
  Unit unit(component);
  configure(unit, descriptor.type, sampleRate, maxFrames, {});
  checked(AudioUnitInitialize(unit.value), "Initialize Audio Unit");
  unit.initialized = true;
...
std::unique_ptr<PreparedEffect> prepareAudioUnit(const PluginInsert &plugin,
                                                 uint32_t sampleRate,
                                                 uint32_t maxFrames) {
  if (plugin.type == kAudioUnitType_MusicDevice)
    return std::make_unique<AppleMusicDevice>(plugin, sampleRate, maxFrames);
