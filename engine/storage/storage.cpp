...
void writeDraft(const State& state, const std::string& path, SaveObserver observer) {
    ...
    if(observer) observer(1);
    {
        ...
        sql(db.get(), "PRAGMA application_id=1296323159; PRAGMA user_version=24; BEGIN IMMEDIATE;"
            "CREATE TABLE metadata(singleton INTEGER PRIMARY KEY CHECK(singleton=1), revision INTEGER NOT NULL, next_id INTEGER NOT NULL, master_gain REAL NOT NULL, master_solo INTEGER NOT NULL CHECK(master_solo IN(0,1)));"
            ...
            "CREATE TABLE regions(track_id INTEGER NOT NULL, position INTEGER NOT NULL, clip_start INTEGER NOT NULL, source_offset INTEGER NOT NULL, clip_length INTEGER NOT NULL, fade_in INTEGER NOT NULL, fade_out INTEGER NOT NULL, take_index INTEGER NOT NULL, gain REAL NOT NULL, color INTEGER NOT NULL, muted INTEGER NOT NULL CHECK(muted IN(0,1)), looped INTEGER NOT NULL CHECK(looped IN(0,1)), pan REAL NOT NULL, fade_in_shape INTEGER NOT NULL DEFAULT 0, fade_out_shape INTEGER NOT NULL DEFAULT 0, auto_fade_in INTEGER NOT NULL CHECK(auto_fade_in IN(0,1)), auto_fade_out INTEGER NOT NULL CHECK(auto_fade_out IN(0,1)), PRIMARY KEY(track_id,position));"
            "CREATE TABLE buses(position INTEGER PRIMARY KEY, id INTEGER UNIQUE NOT NULL, name TEXT NOT NULL, gain REAL NOT NULL, pan REAL NOT NULL, muted INTEGER NOT NULL CHECK(muted IN(0,1)), solo INTEGER NOT NULL CHECK(solo IN(0,1)), output_bus INTEGER NOT NULL);"
        ...
        sqlite3_bind_int64(meta.get(), 2, static_cast<int64_t>(state.nextID));sqlite3_bind_double(meta.get(),3,state.masterGain);sqlite3_bind_int(meta.get(),4,state.masterSolo); done(meta.get());
        ...
        for (const auto& t : state.tracks) {
            ...
            sqlite3_bind_double(row.get(),6,t.pan);sqlite3_bind_int(row.get(),7,t.muted);sqlite3_bind_int(row.get(),8,t.solo);sqlite3_bind_int64(row.get(),9,static_cast<int64_t>(t.baseStart));sqlite3_bind_int64(row.get(),10,static_cast<int64_t>(t.outputBus));sqlite3_bind_int64(row.get(),11,static_cast<int64_t>(t.color));
            done(row.get());
...
State readDraft(const std::string& path) {
    ...
    if(formatVersion<1 || formatVersion>24) throw Error("Unsupported draft version");
    ...
    auto meta = prepare(db.get(),formatVersion>=22?"SELECT singleton,revision,next_id,master_gain,master_solo FROM metadata":(formatVersion>=7?"SELECT singleton,revision,next_id,master_gain,0 FROM metadata":"SELECT singleton,revision,next_id,0,0 FROM metadata"));
    ...
    const auto masterSolo=integer(meta.get(),4);if(masterSolo<0||masterSolo>1)throw Error("Invalid master solo state");
    State state; state.revision = static_cast<uint64_t>(integer(meta.get(), 1)); state.nextID = static_cast<uint64_t>(integer(meta.get(), 2));state.masterGain=sqlite3_column_double(meta.get(),3);state.masterSolo=masterSolo!=0;
    ...
    else trackQuery = formatVersion>=19 ? "SELECT position,id,name,gain,pcm,pan,muted,solo,base_start,output_bus,color FROM tracks ORDER BY position" : "SELECT position,id,name,gain,pcm,pan,muted,solo,base_start,output_bus FROM tracks ORDER BY position";
    ...
    while ((rc = sqlite3_step(row.get())) == SQLITE_ROW) {
        ...
        state.tracks.push_back({static_cast<uint64_t>(integer(row.get(), 1)), string(row.get(), 2), sqlite3_column_double(row.get(), 3), formatVersion>=19?static_cast<uint32_t>(integer(row.get(),10)):0u, {}, {},formatVersion==3?0:sqlite3_column_double(row.get(),5),muted!=0,solo!=0,formatVersion>=8?static_cast<uint64_t>(integer(row.get(),8)):0,{},formatVersion>=9?static_cast<uint64_t>(integer(row.get(),9)):0,{},{},{},{},{}});
    ...
    if(formatVersion>=4) {
        auto regions=prepare(db.get(),formatVersion==4 ? "SELECT track_id,position,clip_start,source_offset,clip_length,0,0,0 FROM regions ORDER BY track_id,position" : (formatVersion<8?"SELECT track_id,position,clip_start,source_offset,clip_length,fade_in,fade_out,0 FROM regions ORDER BY track_id,position":(formatVersion>=24?"SELECT track_id,position,clip_start,source_offset,clip_length,fade_in,fade_out,take_index,gain,color,muted,looped,pan,fade_in_shape,fade_out_shape,auto_fade_in,auto_fade_out FROM regions ORDER BY track_id,position":(formatVersion>=23?"SELECT track_id,position,clip_start,source_offset,clip_length,fade_in,fade_out,take_index,gain,color,muted,looped,pan,fade_in_shape,fade_out_shape FROM regions ORDER BY track_id,position":(formatVersion>=21?"SELECT track_id,position,clip_start,source_offset,clip_length,fade_in,fade_out,take_index,gain,color,muted,looped,pan FROM regions ORDER BY track_id,position":(formatVersion>=20?"SELECT track_id,position,clip_start,source_offset,clip_length,fade_in,fade_out,take_index,gain,color,muted,looped FROM regions ORDER BY track_id,position":"SELECT track_id,position,clip_start,source_offset,clip_length,fade_in,fade_out,take_index,gain,color FROM regions ORDER BY track_id,position"))))));
    ...
    if(formatVersion>=9){
        auto buses=prepare(db.get(),formatVersion>=22?"SELECT position,id,name,gain,pan,muted,solo,output_bus FROM buses ORDER BY position":"SELECT position,id,name,gain,pan,muted,0,output_bus FROM buses ORDER BY position");
        while((rc=sqlite3_step(buses.get()))==SQLITE_ROW){if(integer(buses.get(),0)!=static_cast<int64_t>(state.buses.size()))throw Error("Invalid draft bus order");const auto muted=integer(buses.get(),5),solo=integer(buses.get(),6);if(muted<0||muted>1||solo<0||solo>1)throw Error("Invalid bus mute or solo state");state.buses.push_back({static_cast<uint64_t>(integer(buses.get(),1)),string(buses.get(),2),sqlite3_column_double(buses.get(),3),sqlite3_column_double(buses.get(),4),muted!=0,solo!=0,static_cast<uint64_t>(integer(buses.get(),7)),{},{}});}
    ...
    if(formatVersion>=16){
        auto midiClips=prepare(db.get(),formatVersion>=19?"SELECT track_id,position,start,length,lane,color FROM midi_clips ORDER BY track_id,position":"SELECT track_id,position,start,length,lane FROM midi_clips ORDER BY track_id,position");
        while((rc=sqlite3_step(midiClips.get()))==SQLITE_ROW){const auto trackID=static_cast<uint64_t>(integer(midiClips.get(),0));auto track=std::find_if(state.tracks.begin(),state.tracks.end(),[&](const auto& item){return item.id==trackID;});if(track==state.tracks.end()||integer(midiClips.get(),1)!=static_cast<int64_t>(track->midiClips.size()))throw Error("Invalid draft MIDI clip order");const auto lane=integer(midiClips.get(),4);if(lane<0)throw Error("Invalid MIDI clip lane");track->midiClips.push_back({static_cast<uint64_t>(integer(midiClips.get(),2)),static_cast<uint64_t>(integer(midiClips.get(),3)),{},static_cast<int>(lane),formatVersion>=19?static_cast<uint32_t>(integer(midiClips.get(),5)):0u});}
