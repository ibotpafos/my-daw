...
void writeDraft(const State& state, const std::string& path, SaveObserver observer) {
    ...
    if(observer) observer(1);
    {
        ...
        sql(db.get(), "PRAGMA application_id=1296323159; PRAGMA user_version=22; BEGIN IMMEDIATE;"
        ...
        for(const auto& track:state.tracks)for(size_t position=0;position<track.sends.size();++position){const auto& send=track.sends[position];sqlite3_reset(sendRow.get());sqlite3_clear_bindings(sendRow.get());sqlite3_bind_int64(sendRow.get(),1,static_cast<int64_t>(track.id));sqlite3_bind_int64(sendRow.get(),2,static_cast<int64_t>(position));sqlite3_bind_int64(sendRow.get(),3,static_cast<int64_t>(send.bus));sqlite3_bind_double(sendRow.get(),4,send.gain);sqlite3_bind_int(sendRow.get(),5,send.preFader);sqlite3_bind_double(sendRow.get(),6,send.pan);sqlite3_bind_int(sendRow.get(),7,send.muted);sqlite3_bind_int(sendRow.get(),8,send.independentPan);done(sendRow.get());}
...
State readDraft(const std::string& path) {
    ...
    if(formatVersion<1 || formatVersion>22) throw Error("Unsupported draft version");
    ...
    if(formatVersion>=4) {
        auto regions=prepare(db.get(),formatVersion==4 ? "SELECT track_id,position,clip_start,source_offset,clip_length,0,0,0 FROM regions ORDER BY track_id,position" : (formatVersion<8?"SELECT track_id,position,clip_start,source_offset,clip_length,fade_in,fade_out,0 FROM regions ORDER BY track_id,position":(formatVersion>=21?"SELECT track_id,position,clip_start,source_offset,clip_length,fade_in,fade_out,take_index,gain,color,muted,looped,pan FROM regions ORDER BY track_id,position":(formatVersion>=20?"SELECT track_id,position,clip_start,source_offset,clip_length,fade_in,fade_out,take_index,gain,color,muted,looped FROM regions ORDER BY track_id,position":"SELECT track_id,position,clip_start,source_offset,clip_length,fade_in,fade_out,take_index,gain,color FROM regions ORDER BY track_id,position"))));
