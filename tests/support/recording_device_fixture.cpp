...
public:
    ...
    void start() override {
        ...
        capture_ = std::make_unique<daw::DuplexCapture>(renderer, state_, capacity_, path_, start_, loopStart_, loopEnd_, preroll_, monitor_, latency_, inputChannels_);
        renderer.playing = true; active_ = true; current = this;
    }
    void pump(const float* input, const float* inputRight, uint32_t frames, float* left, float* right) {
        ...
        capture_->process(input, left, right, frames, time, inputTime, inputChannels_ == 2 ? inputRight : nullptr);
...
std::unique_ptr<Duplex> makeDuplex(const State& state, uint64_t capacity, const std::string& path,
    ...
    return std::make_unique<DeviceFixture>(state, capacity, path, start, loopStart, loopEnd, preroll, monitor, config.inputChannels);
}
}
extern "C" int recording_fixture_pump(const float* input, uint32_t frames, float* left, float* right) {
    ...
    try { current->pump(input, nullptr, frames, left, right); return 0; } catch (...) { return 1; }
}
extern "C" int recording_fixture_pump_stereo(const float* inputLeft, const float* inputRight,
    ...
    try { current->pump(inputLeft, inputRight, frames, left, right); return 0; } catch (...) { return 1; }
}
