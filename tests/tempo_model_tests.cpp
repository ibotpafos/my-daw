...
int main(){try{
    ...
    // ---- Storage: v17 round trip, v16 fallback to defaults, corrupt maps rejected ----
    {   Session w; w.add("Wired",0);
        ...
        CHECK(sqlite3_step(version)==SQLITE_ROW&&sqlite3_column_int(version,0)==17);
    ...
    std::cout << "PASS: tempo model: frame-0 anchored tempo and time-signature maps, four revision-checked commands, invertible beat/frame conversion, undo/redo, writeDraft/readDraft round trip at schema v17 with v16 default fallback and bridge smoke" << std::endl;
    return 0;
