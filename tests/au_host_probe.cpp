...
int main(){try{
    ...
    for(const auto& component:catalog){
        ...
        for(uint64_t block=0;block<4;++block){ok=effect->process(left.data(),right.data(),512,block*512,{},midi)&&ok;for(size_t i=0;i<left.size();++i){if(!std::isfinite(left[i])||!std::isfinite(right[i]))throw daw::Error("Audio Unit returned non-finite audio");peak=std::max({peak,std::abs(left[i]),std::abs(right[i])});}std::fill(left.begin(),left.end(),0);std::fill(right.begin(),right.end(),0);}
