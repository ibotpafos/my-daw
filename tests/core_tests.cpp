...
int main() { try {
    ...
    daw_insert_hosting_status vst3Hosting{};vst3Hosting.struct_size=sizeof(vst3Hosting);CHECK(daw_get_insert_hosting_status(hostingBridge.get(),DAW_INSERT_OWNER_MASTER,0,3,&vst3Hosting)==0&&vst3Hosting.version==DAW_INSERT_HOSTING_STATUS_VERSION&&vst3Hosting.format==DAW_INSERT_HOSTING_FORMAT_VST3&&vst3Hosting.selected_mode==DAW_INSERT_HOSTING_MODE_IN_PROCESS&&vst3Hosting.supported_modes==DAW_INSERT_HOSTING_MODE_FLAG_IN_PROCESS);
    CHECK(daw_set_insert_hosting_mode(hostingBridge.get(),DAW_INSERT_OWNER_MASTER,0,3,DAW_INSERT_HOSTING_MODE_OUT_OF_PROCESS,unchangedHostingSnapshot.revision)==1);unchangedHostingSnapshot.struct_size=sizeof(unchangedHostingSnapshot);CHECK(daw_get_snapshot(hostingBridge.get(),&unchangedHostingSnapshot)==0&&unchangedHostingSnapshot.revision==hostingSnapshot.revision);
