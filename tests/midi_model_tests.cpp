...
int main(){try{
    ...
    std::cout << "PASS: MIDI model: clips/notes validated, seven revision-checked commands (appendMidiNotes keeps capture order and never grows a clip), undo/redo and writeDraft/readDraft round trip at schema v19 with v15/v16/v17/v18 fallback, MidiRecorder takes and the batch fit predicate" << std::endl;
    return 0;
