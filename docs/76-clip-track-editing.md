...
## Что это
Домашняя арка «удобно как в любой DAW»: клип и дорожка получают полный набор свойств и операций уровня редактора — цвет, clip gain (dB), mute/solo/gain/duplicate дорожки, цвет/transpose/quantize MIDI-клипов, буфер обмена и перенос между дорожками, состояние воспроизведения клипа (mute/loop, черновик v20) — через domain-команды, C-ABI и AppKit UI. Деструктивности нет: всё — ревизии + один Undo-шаг.
...
## Состояние воспроизведения клипа (1.52.0)
Регион получил два флага: `muted` и `looped` (схема черновика v20 — две новые колонки в `regions` с CHECK(0,1), читатель гейтит их по `formatVersion>=20`, старые черновики открываются без изменений). Мьют вынимает регион из render-плана целиком — голос не заводится, кроссфейд-соседи не слышат «дырку» вместо него; полностью замьюченный проект честно отказывает prepare'ом («Import audio or add…»). Луп даёт региону право пережить свой срез: `validate()` при `looped` снимает ограничение `length ≤ frames−sourceOffset`, рендерер оборачивает чтение по модулю длины среза — рампа из 1000 кадров слышна трижды в окне 1500 и совпадает кадр в кадр. Разлуп длинного региона — честный reject через validate (ревизия не тратится). Split залупленного региона отклоняется («Unloop the clip before splitting») — правая половина ушла бы за конец среза; команды `setClipMuted`/`setClipLooped` — revision-checked с молчаливым no-op, мост `daw_set_clip_muted`/`daw_set_clip_looped` (валидация флага 0/1 на границе ABI) + два поля `muted`/`looped` в конце `daw_clip`.
Заодно исправлены два регресса v19: позиционные агрегат-инициализаторы в `editClipFull` и `splitClip` пересоздавали Region шестью элементами и роняли gain/color на каждом триме и сплите — теперь обе операции переносят все десять полей; тесты закрепляют carry («split preserves gain+color», trim держит цвет).
UI: «Мьют клипа (M)» и «Луп клипа (L)» в контекстном меню клипа с галочками состояния; те же буквы — хоткеи в фокусе волны; залупленный клип рисуется с «↻» в подписи, замьюченный — с «MUTE» и телом на четверть прозрачнее; drag-trim сохраняет флаги в превью.

## Мультиселект группы (1.55.0)
Доменные команды `deleteClips(id, indices, expected)` и `nudgeClips(id, indices, deltaFrames, expected)` — вся группа в одной ревизии: сортировка/уникализация индексов, отказ «один плохой индекс отравляет весь батч» до изменений, nudge клампит по таймлайну («No timeline space»), пересобирает порядок по start и оставляет пересечения/кроссфейды на `validate()`; пустая группа — молчаливый no-op до поиска дорожки (консистентно с quantize). Мост `daw_delete_clips` / `daw_nudge_clips` (массив+count, NULL при ненулевом count — отказ). UI: Ctrl-клик тоггит клип в группе (последний снять нельзя), группа подсвечивается как выбранная, Delete над группой >1 удаляет её одной ревизией (одиночный — прежняя команда с прежним сообщением), Option+←/→ двигает группу на шаг beat-grid, в контекстном меню группы появляются «Удалить группу (N)» и два сдвига. Попутно закрыт пропуск 1.52.0: клавиши M/L наконец маршрутизируются из keyDown волны (раньше были только в контроллере, до волны не доходили).

## Панорама клипа (1.59.0)

Регион получил одиннадцатое поле `pan` (−1..1, ноль — центр) и команду `setClipPan`: тот же линейный закон с unity-центром, что и панорама дорожки (`L=1−pan`, `R=1+pan` при своих знаках) — центр не роняет уровень на −3 dB, панорама клипа перемножается с панорамой дорожки в голосе рендера (коэффициенты сворачиваются в prepare, колбэк только умножает). Pan переживает split/duplicate/editClip как gain и color, валидируется `Clip pan outside -1..1` (NaN/inf туда же), reject не тратит ревизию. Схема v21: колонка `pan REAL NOT NULL` в regions (CHECK в SQLite-ALTER не выразить — диапазон держат домен и clamp рендера), чтение v≤20 даёт 0.0, future-гейт сдвинут на 21. Мост: `daw_clip.pan` (в конец структуры) и `daw_set_clip_pan`. UI: подменю «Панорама клипа» из пяти пресетов с галочкой текущего значения и метка `P L50%` в теге клипа.

## Тесты
`clip_track_model` (17-й CTest, 84 CHECK): все одиннадцать команд — ревижн-матрица, no-op'ы, undo/redo до состояния импорта и обратно, duplicateTrack (новый id, «copy» в имени, копия regions/MIDI с цветами; `Track not found`; общий cap 256 не гонялся — унаследован), transpose с клампом обоими краями (127 сверху, 0 снизу), quantize (on-grid не двигается, 0.75 бит → вверх, grid≤0 no-op), storage round-trip v20 с обоими цветами, −9 dB и флагами mute/loop. Секции clip-gain (−6.0206 dB = ровно ×0.5 на миксе) и fade-in огибающей — в `audio_render_storage`; clip/track sequence + C-ABI негативы — в `pure_c_bridge`.
