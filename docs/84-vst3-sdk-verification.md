...
## Время жизни результатов сканирования

`PluginFactory::classInfos()` возвращает вектор по значению. Раньше probe
сохраняла указатель на элемент временного range-for и использовала его уже
после конца цикла. Список теперь хранится до создания выбранного компонента;
ADelay scanner/probe проверяет этот путь на реальном SDK. ASan/UBSan включены
также для VST3 scanner/runtime helpers: они не линкуют ядро и прежде не
наследовали его sanitizer flags. Тестовый launcher сохраняет stderr дочернего
процесса в отчёте при ошибке. Сам upstream ADelay собирается обычной SDK-сборкой,
а не объявляется полностью инструментированным сторонним бинарником.
