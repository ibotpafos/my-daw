...
явный Mono/Stereo вход и независимые L/R каналы stereo master. Числа в интерфейсе начинаются
...
Запись использует выбранный Mono-канал либо явную пару Input L/R. После P0-04
нативный `AudioDeviceIOProc` читает выбранные hardware buffers напрямую; Stereo
не суммирует каналы и отклоняет несинхронную пару. Output map по-прежнему направляет
master только в выбранные L/R, остальные destinations молчат. Собственный
ресемплер/DSP для device routing не добавлен.
...
Обычная и loop-запись требуют одного физического либо заранее созданного
aggregate-устройства для input/output. Независимые clocks не синхронизируются
тайно. Stereo pair остаётся внутри той же clock domain.
...
`audio.devices.v2` — bounded versioned JSON (v1 мигрирует в Mono) в UserDefaults, не SQLite проекта.
