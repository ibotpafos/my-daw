...
а не выдуманную измеренную round-trip latency. Первоначальный DEV-01 срез
только читал эти значения. Следующий [P0-02 срез](94-audio-device-rate-buffer.md)
добавляет отдельные кнопки частоты/буфера для входа и выхода: явное применение
48 kHz и допустимого размера с HAL notification/readback и timeout, без скрытой
автоматической настройки. Audio MIDI Setup/vendor panel остаются внешней альтернативой. Неподходящая частота или buffer >4096 запрещают запуск.
