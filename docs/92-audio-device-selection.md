...
Full-duplex запись требует одного физического либо заранее созданного
aggregate-устройства для input/output. Независимые clocks не синхронизируются
тайно. [Обычная запись](95-normal-duplex-recording.md) использует этот же путь
с backing playback, MON и preroll; input-only fallback в приложении отсутствует.
