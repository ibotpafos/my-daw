...
[1.74.0](79-command-palette.md) — палитра команд из меню «Вид» и по `⌘K`, поиск, последние/частые действия, ограниченная локальная история, повторная проверка menu-context и native AppKit regression harness. Исправлены Linux/GCC build blockers без ослабления предупреждений. Ручной полный VoiceOver audit и физический keyboard/IME smoke остаются отдельными gates.
...
[1.73.0](12-development.md) — наблюдаемость вместо догадок: единая точка запуска `scripts/run-macos.sh` (`--verify`, `--logs`, `--telemetry`, `--debug`), unified log приложения по категориям Lifecycle/Bridge/Audio/Jobs/Plugins и версия билда из `VERSION` с ревизией git в `DAWBuildCommit`.
...
[1.62.0](74-markers.md) — «Цикл отсюда до следующего маркера» в контекстном меню локатора с синхронизацией оверлея диапазона.

[1.61.0](68-midi-foundation.md) — добавление и удаление MIDI-клипов из пиано-ролла: «+ Клип» на позиции воспроизведения (снап к полутакту, 4 такта), «− Клип» выбранного.
