...
## Меню-хоткеи не воюют с набором текста и с монтажом (1.72.0)

Соло/мьют/арм и play/stop добавлены пунктами главного меню, а эквивалент меню обрабатывается до `view.keyDown` — то есть раньше и поля ввода, и слоя волны. Поэтому введены два ограничения:

- `validateMenuItem` отключает Space/⌥S/⌥M/⌥A, пока первым ответчиком окна является текстовый редактор (`NSTextView` или `currentEditor()` контрола). Переименование дорожки, имя маркера и числовые поля инспектора набираются как обычно;
- дорожные команды переехали на модификатор: **⌥S**, **⌥M**, **⌥A**. Голые `s`/`m`/`d`/`c`/`v`/`l` остаются монтажными клавишами слоя волны (разрезать клип, mute клипа, дубль, копировать/вставить, loop); сам `WaveformView` теперь требует отсутствия Command, Option и Control, так что ⌥S не режет клип пополам.

Space — настоящий тумблер: при воспроизведении останавливает, при остановке запускает, а во время записи вызывает `finishRecording()`. Раньше он звал `daw_stop` напрямую, что оставляло активный рекордер незавершённым.

Solo/Mute/Arm читают состояние через `daw_get_track` и пишут `daw_set_solo`/`daw_set_mute` (arm — только UI-состояние `armedTrackID`), поэтому команды по-прежнему проходят через revision-aware мост.
