...
## Проверка реальным отказом

Изолированный тестовый процесс с запрещённым чтением памяти создаёт настоящий
macOS IPS. Это намеренный тест диагностики, не воспроизведение дефекта #30.
Самопроверка обнаружила, что `procLaunch` вида
`2026-09-18 06:17:54.2776 +0000` нельзя надёжно читать только через
`datetime.fromisoformat`: нужны Apple-формат с дробными секундами и явный timezone.
Регрессия покрывает UTC, отрицательный и дробный часовой пояс; naive/future
запуски отклоняются. Обычные ISO даты тоже поддерживаются.

`swiftc -g` сам формирует dSYM до удаления временных Swift object files.
Повторный `dsymutil` после завершения driver теряет их DWARF, поэтому сохраняется
исходный dSYM. Перед запуском сравниваются UUID executable/dSYM и проверяются
Swift line tables для `mix_export_ui_tests.swift`, `MixExportUI.swift`,
`StorageUI.swift`; `diagnostics/symbols.json` фиксирует результат.
