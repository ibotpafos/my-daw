...
| P0-04 | Компенсация записи | [Первый срез: контроль callback clock](96-recording-clock.md) — sample/host gaps, отказ commit и recovery. **Вся P0-04 не завершена.** Sample/host timestamps, input/output latency/safety offsets, подтверждённое размещение capture, loop/punch boundaries. PDC плагинов не подменяет этот пункт. |
