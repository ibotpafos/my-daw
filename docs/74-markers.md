...
## Цикл от маркера (1.62.0)

Контекстное меню маркера-локатора получило пункт «Цикл отсюда до следующего»: правый клик по флажку ставит `daw_set_loop(1, marker, next)` на ближайший маркер правее (своих изменений движка нет — валидация «Loop range must be inside the project» и гейт записи штатные), а UI-оверлей цикла перекрашивается: `rangeStart/rangeEnd` синхронизируются с границами цикла, как это делает кнопка «↻ Цикл». Если правее маркера нет — сообщение «Нет маркера правее», цикл не трогается. Пункт в том же меню, что переход/переименование/удаление; горячая раскладка прежняя.
